...
export function registerIpcHandlers(dependencies:{
  ...
  ipcMain.handle('pos:add-cash-operation',(_event,type:CashOperationType,amountMinor:number,reason:string,cleaningPayoutId?:string)=>{assertCashierAccess();return database.addCashOperation(type,amountMinor,reason,cleaningPayoutId)})
  ...
  ipcMain.handle('pos:prepare-cleaner-payout',(_event,cycleId:string)=>{assertCashierAccess();return database.prepareCleanerPayout(cycleId)})
