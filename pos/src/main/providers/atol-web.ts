...
export class AtolWebFiscalProvider implements FiscalProvider {
  constructor(
    ...
    private readonly manager?: AtolWebManager,
  ...
  async healthCheck(): Promise<DeviceHealth> {
    ...
    if (!settings.enabled)
      return {
        ...
        message: "АТОЛ 1Ф не включён в настройках",
      };
    try {
  ...
  async getShiftStatus(): Promise<FiscalShiftStatus> {
    const task = await this.execute(
      randomUUID(),
      { type: "getShiftStatus" },
      8000
    );
    const state = String(
      (task.result?.shiftStatus as Record<string, unknown> | undefined)
        ?.state ?? "unknown"
    );
    if (state === "opened")
      return {
        open: true,
        state: "opened",
        message: "АТОЛ готов · смена открыта",
      };
    if (state === "closed")
      return {
        open: false,
        state: "closed",
        message: "АТОЛ готов · смена закрыта",
      };
    if (state === "expired")
      return {
        open: true,
        state: "expired",
        message: "Фискальная смена истекла и требует закрытия",
      };
  ...
  async closeShift(
    ...
    return {
      message: "Фискальная смена закрыта",
      reportNumber: this.pickString(result, [
  ...
  async getOperationStatus(request: {
    ...
    try {
      ...
      if (!task)
        return {
          status: "unknown",
          message: "ATOL Web Server ещё не вернул результат операции",
        };
      if (task.error && Number(task.error.code ?? 0) !== 0)
        return {
          status: "not_found",
          message: task.error.description || "Фискальная операция не выполнена",
          raw: task,
        };
      ...
      return parsed
        ...
        : {
            status: "unknown",
            message:
              "ATOL выполнил задачу, но фискальный номер пока не найден в ответе",
            raw: task,
          };
    } catch (error) {
      return {
    ...
    if (task.error && Number(task.error.code ?? 0) !== 0)
      throw new Error(
        task.error.description ||
          `АТОЛ не выполнил фискальную операцию ${operationId}`
      );
    const result = this.tryParseFiscalResult(task);
    if (!result)
      throw new Error(
        "АТОЛ сообщил об окончании операции, но не вернул идентификатор фискального документа"
      );
    return result;
  }
  ...
  private tryParseFiscalResult(task: AtolTaskResult): FiscalResult | undefined {
    const result = task.result ?? {};
    const fiscalParams =
      (result.fiscalParams as Record<string, unknown> | undefined) ?? {};
    const receiptNumber =
      this.pickString(fiscalParams, [
        "fiscalDocumentNumber",
        "documentNumber",
        "receiptNumber",
      ]) ??
      this.pickString(result, [
        "fiscalDocumentNumber",
        "documentNumber",
        "receiptNumber",
      ]);
    ...
    return {
      ...
      fiscalDocumentNumber: this.pickString(fiscalParams, [
        "fiscalDocumentNumber",
      ]),
      fiscalSign: this.pickString(fiscalParams, [
        "fiscalSign",
        "fiscalSignShort",
      ]),
      shiftNumber:
        this.pickString(fiscalParams, ["shiftNumber"]) ??
        this.pickString(result, ["shiftNumber"]),
      raw: task,
    };
  }
  ...
  private async execute(
    ...
    await this.fetchJson(
      `${settings.web.baseUrl}/requests`,
      {
        ...
        body: JSON.stringify({ uuid, request: [request] }),
      },
      5000
    );
    ...
    throw new Error(
      `АТОЛ не подтвердил завершение операции ${uuid} за ${Math.round(
        waitMs / 1000
      )} сек.`
    );
  }
