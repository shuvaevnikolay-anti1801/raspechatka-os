...
describe("InpasPaymentProvider", () => {
  ...
  it("clears stale result before launching and locks concurrent operations", async () => {
    writeFileSync(join(directory, "result.txt"), "[39] = '1'");
    ...
    const provider = new InpasPaymentProvider(
      ...
      async (_file, _args, options) => {
        expect(() => readFileSync(join(options.cwd, "result.txt"))).toThrow();
        await gate;
        writeFileSync(join(options.cwd, "result.txt"), "[39] = '1'");
        return {
          code: 0,
          signal: null,
          stdout: "",
    ...
    ).rejects.toThrow("другую операцию");
    release();
    await first;
  });
});
