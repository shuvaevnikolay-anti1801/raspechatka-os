...
const invokeShift = async <T>(
  channel: "pos:open-shift" | "pos:close-shift"
): Promise<T> => {
  try {
...
const api: ExtendedPosApi = {
  ...
  recordShiftDiscrepancy: (differenceMinor: number, note: string) =>
    ipcRenderer.invoke("pos:record-shift-discrepancy", {
      differenceMinor,
      note,
    }),
  ...
  holdReceipt: (receipt: Omit<HeldReceipt, "id" | "createdAt">) =>
    ipcRenderer.invoke("pos:hold-receipt", receipt),
  deleteHeldReceipt: (id: string) =>
    ipcRenderer.invoke("pos:delete-held-receipt", id),
  ...
  saveConnection: (config: ConnectionConfig) =>
    ipcRenderer.invoke("pos:save-connection", config),
  getCashierAuthState: () => ipcRenderer.invoke("pos:get-cashier-auth-state"),
  beginCashierLogin: (cashierId: string) =>
    ipcRenderer.invoke("pos:begin-cashier-login", cashierId),
  createCashierPin: (cashierId: string, pin: string, confirmation: string) =>
    ipcRenderer.invoke("pos:create-cashier-pin", cashierId, pin, confirmation),
  loginCashier: (cashierId: string, pin: string) =>
    ipcRenderer.invoke("pos:login-cashier", cashierId, pin),
  ...
  verifyAdminCode: (code: string) =>
    ipcRenderer.invoke("pos:verify-admin-code", code),
  resetCashierPin: (
    cashierId: string,
    adminCode: string,
    pin: string,
