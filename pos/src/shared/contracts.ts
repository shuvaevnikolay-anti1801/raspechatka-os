...
export type InpasDirectDevice = {terminalId:string;model?:string;serial?:string}
export type InpasConsoleConfig = {executablePath:string;terminalId:string;currencyCode:string;timeoutMs:number;qrMode:'terminal_choice'}
export type InpasSettings = {version:2;enabled:boolean;adapter:'direct'|'console';direct?:{selectedDevice?:InpasDirectDevice};console:InpasConsoleConfig}
export type InpasDriverInfo = {installed:boolean;version?:string;architecture?:'x64';code?:string;error?:string}
export type InpasDirectStatus = {installed:boolean;status:string;version?:string}
export type InpasConnectionResult = {success:boolean;status:string;terminalId:string;referenceNumber?:string;terminalTransactionId?:string;model?:string;serial?:string;responseCode?:string;responseDescription?:string;transactionStatus?:string;receipt?:string}
export type InpasDiscoveryResult = {state:'ready'|'not_initialized'|'driver_missing';message:string;devices:InpasDirectDevice[];driver:InpasDriverInfo}
...
export type PosApi = {
  ...
  getInpasDriverInfo: () => Promise<InpasDriverInfo>
  discoverInpasDevices: () => Promise<InpasDiscoveryResult>
  selectInpasDevice: (device:InpasDirectDevice) => Promise<InpasSettings>
  testInpasDirectDevice: () => Promise<InpasConnectionResult>
