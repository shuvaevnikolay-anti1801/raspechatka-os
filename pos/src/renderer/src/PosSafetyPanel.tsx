...
const recoveryText = (
...
): { title: string; detail: string; critical: boolean } => {
  if (
    operation.state === "payment_unknown" ||
  ...
  if (
    operation.state === "fiscal_status_unknown" ||
    operation.state === "fiscalization_in_progress"
  )
    return {
      title: "Состояние фискального чека неизвестно",
      detail:
        "Не пробивайте второй чек вручную. Касса сначала запросит результат уже начатой операции АТОЛ.",
      critical: true,
    };
  if (operation.state === "fiscalized")
    return {
      title: "Фискальный чек уже пробит",
      detail:
        "Повторная фискализация запрещена. Нужно только завершить локальную запись операции.",
      critical: false,
    };
  return {
    title:
      operation.kind === "sale"
        ? "Продажу нужно завершить"
        : "Возврат нужно завершить",
    detail:
      operation.lastError ||
      "Операция сохранена локально и может быть безопасно продолжена.",
    critical: false,
  };
};
...
export default function PosSafetyPanel() {
  ...
  const selectPrinter = async (name: string) => {
    try {
      ...
      setMessage("Товарный принтер сохранён");
      await refresh();
    } catch (error) {
  ...
  const saveAtol = async () => {
    try {
      ...
      setAtol(saved);
      setMessage("Настройки АТОЛ сохранены. Проверяем связь с ККТ…");
      await refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
    }
  ...
  const terminalAction = async (action: "test" | "reconcile") => {
    try {
      setMessage(
        action === "test"
          ? "Проверяем связь с PAX…"
          : "Выполняем сверку итогов…"
      );
      const result =
        action === "test"
  ...
  const statusClass = (ready: boolean, status?: string) =>
    ready ? "ok" : status === "not_configured" ? "muted" : "bad";
  ...
  return (
    <>
      <aside className="safety-strip" aria-label="Состояние кассы">
        <Status
          label="OS"
          value={devices?.os.message || "Проверяем…"}
          state={statusClass(Boolean(devices?.os.ready), devices?.os.status)}
        />
        <Status
          label="ККТ"
          value={devices?.fiscal.message || "Проверяем…"}
          state={statusClass(
            Boolean(devices?.fiscal.ready),
            devices?.fiscal.status
          )}
        />
        <Status
          label="Терминал"
          value={devices?.payment.message || "Проверяем…"}
          state={statusClass(
            Boolean(devices?.payment.ready),
            devices?.payment.status
          )}
        />
        <Status
          label="Принтер"
          value={devices?.printer.message || "Проверяем…"}
          state={statusClass(
            Boolean(devices?.printer.ready),
            devices?.printer.status
          )}
        />
        <Status
          label="Смена"
          value={devices?.shift.message || "Проверяем…"}
          state={devices?.shift.ready ? "ok" : "bad"}
        />
        <button
          className={
            attentionCount ? "recovery-button danger" : "recovery-button"
          }
      ...
      {open && (
        ...
        >
          <section className="safety-panel">
            <header>
              <div>
                <small>НАДЁЖНОСТЬ КАССЫ</small>
                <h2>Оборудование и восстановление</h2>
              </div>
            ...
            >
              <div>
                <small>СОСТОЯНИЕ СМЕНЫ</small>
                <strong>
                  {devices?.shift.message ||
                    "Проверяем состояние локальной и фискальной смены…"}
                </strong>
              </div>
              {!devices?.shift.ready && (
                <p>
                  До начала продаж нужно устранить несоответствие локальной
                  смены и смены ККТ.
                </p>
              )}
            </section>
              ...
              <div className="settings-title">
                ...
                <label className="toggle">
                  <input
                    ...
                    onChange={(event) =>
                      setAtol({ ...atol, enabled: event.target.checked })
                    }
                  />
                  <span>Использовать АТОЛ</span>
                </label>
              </div>
              <div className="settings-grid">
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setAtol({ ...atol, taxationType: event.target.value })
                    }
                  >
                ...
                <label>
                  <span>НДС позиции</span>
                  <select
                    value={atol.taxType}
                    onChange={(event) =>
                      setAtol({ ...atol, taxType: event.target.value })
                    }
                  >
                ...
                <label>
                  <span>Оператор ККТ</span>
                  <strong>Текущий кассир</strong>
                </label>
              </div>
              <button className="save-hardware" onClick={saveAtol}>
            ...
            <section className="hardware-settings inpas-settings">
              <div className="settings-title">
                <div>
                  <h3>Точка / INPAS · PAX</h3>
                  <p>
                    Касса запускает официальный DC Console.exe отдельным
                    процессом. Для карты и QR используется банковский экран
                    терминала.
                  </p>
                </div>
                <label className="toggle">
                  <input
                    ...
                    onChange={(event) =>
                      setInpas({ ...inpas, enabled: event.target.checked })
                    }
                  />
                  <span>Использовать INPAS</span>
                </label>
              </div>
              <div className="settings-grid">
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setInpas({ ...inpas, executablePath: event.target.value })
                    }
                    placeholder="Определяется автоматически или укажите вручную"
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setInpas({ ...inpas, terminalId: event.target.value })
                    }
                    placeholder="Например, из настройки банка"
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setInpas({ ...inpas, currencyCode: event.target.value })
                    }
                    inputMode="numeric"
                  />
                </label>
                <label>
                  <span>Ожидание операции, секунд</span>
                  <input
              ...
              <p className="hardware-note">
                Важно: комплект INPAS использует одну команду продажи для карты
                и QR. При выборе «QR / СБП» касса передаёт сумму терминалу, а
                конкретный способ подтверждается на экране PAX.
              </p>
              <div className="hardware-actions">
                <button className="save-hardware" onClick={saveInpas}>
                  Сохранить INPAS
                </button>
                <button onClick={() => void terminalAction("test")}>
                  Проверить связь
                </button>
                <button onClick={() => void terminalAction("reconcile")}>
                  Сверка итогов
                </button>
              </div>
            </section>
            <section className="printer-settings">
              <div>
                <h3>Принтер товарного чека</h3>
                <p>
                  Выберите установленный в Windows принтер один раз. Дальше
                  печать идёт на него без системного окна.
                </p>
              </div>
              <select
            ...
            <section className="recovery-list">
              ...
              {!unresolved.length ? (
                <div className="recovery-empty">
                  ...
                  <span>
                    Нет операций с неизвестным состоянием оплаты или ККТ.
                  </span>
                </div>
              ) : (
                unresolved.map((operation) => {
                  const text = recoveryText(operation);
                  return (
                    <article
                      key={operation.id}
                      className={text.critical ? "critical" : ""}
                    >
                      <div>
                        <b>{text.title}</b>
                        <small>
                          {operation.kind === "sale" ? "Продажа" : "Возврат"} ·{" "}
                      ...
                      <div>
                        ...
                        {operation.lastError && (
                          <small>{operation.lastError}</small>
                        )}
                      </div>
                      <button onClick={() => recover(operation.id)}>
            ...
            <section className="recovery-list print-recovery-list">
              ...
              {!printJobs.length ? (
                <div className="recovery-empty">
                  ...
                  <span>Все товарные чеки напечатаны.</span>
                </div>
              ) : (
                printJobs.map((job) => (
                  <article
                    key={job.id}
                    className={job.state === "error" ? "critical" : ""}
                  >
                    <div>
                      <b>Товарный чек не напечатан</b>
                      <small>
                    ...
                    <div>
                      <strong>
                        Продажа уже сохранена и не будет отменена.
                      </strong>
                      {job.lastError && <small>{job.lastError}</small>}
                    </div>
                    <button onClick={() => retryPrint(job.id)}>
                      Повторить печать
                    </button>
                  </article>
                ))
...
function Status({
  ...
  return (
    <div className={"safety-status " + state} title={value}>
      <i />
      <span>{label}</span>
      <small>{value}</small>
    </div>
...
function DeviceCard({
  ...
  title: string;
  status: DeviceStatuses["fiscal"] | undefined;
}) {
  return (
    <article className={status?.ready ? "ready" : "not-ready"}>
      <small>{title}</small>
      <strong>
        {status?.ready
          ? "Готово"
      ...
      <span>{status?.message || "Проверяем…"}</span>
    </article>
  );
}
