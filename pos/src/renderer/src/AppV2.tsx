...
const emptyWorkplace:WorkplaceData={schedule:[],scheduleMonth:{month:'',days:0,employees:[],entries:[]},myUpcomingShifts:[],deliveries:[],supplyRequests:[],cleaner:{visitsSincePayment:0,paymentDueMinor:0,recentVisits:[]},orders:[]}
...
function WorkPage({products,data,shiftOpen,onChanged,notify}:{products:Product[];data:WorkplaceData;shiftOpen:boolean;onChanged:()=>Promise<void>;notify:(text:string)=>void}){
  ...
  const entryFor=(employeeId:string,day:number)=>scheduleMonth.entries.find((entry)=>entry.employeeId===employeeId&&Number(entry.date.slice(-2))===day)
  const stockProducts=products.filter((x)=>x.trackInventory)
  return <Page title="Рабочее место" kicker=""><div className="work-tabs"><button className={tab==='schedule'?'active':''} onClick={()=>setTab('schedule')}>График работы</button><button className={tab==='stock'?'active':''} onClick={()=>setTab('stock')}>Товары и склад</button><button className={tab==='delivery'?'active':''} onClick={()=>setTab('delivery')}>Поставки</button><button className={tab==='cleaner'?'active':''} onClick={()=>setTab('cleaner')}>Уборка{data.cleaner.paymentDueMinor>0&&<b>!</b>}</button></div>
    {tab==='schedule'&&<div className="work-schedule">
      <section className="work-card schedule-upcoming"><h3>Мои ближайшие 5 смен</h3>{data.myUpcomingShifts.length?data.myUpcomingShifts.map((entry)=><article key={entry.id}><div><b>{new Date(entry.date+'T00:00:00').toLocaleDateString('ru-RU',{weekday:'short',day:'numeric',month:'short'})}</b><small>{entry.shiftCode||entry.shiftName}</small></div><strong>{entry.startTime.slice(0,5)}–{entry.endTime.slice(0,5)} · {entry.plannedHours} ч.</strong></article>):<p>Ближайших опубликованных смен нет.</p>}</section>
      <section className="work-card schedule-month"><header><div><h3>График точки</h3><p>{monthLabel}</p></div><small>Только просмотр</small></header>{days.length>0&&scheduleMonth.employees.length?<div className="schedule-grid"><div className="schedule-grid-row schedule-grid-header" style={{gridTemplateColumns:gridColumns}}><strong>Сотрудник</strong>{days.map((day)=>{const date=new Date(scheduleMonth.month+'-'+String(day).padStart(2,'0')+'T00:00:00');return <span key={day}>{date.toLocaleDateString('ru-RU',{weekday:'short'}).replace('.','')}<b>{day}</b></span>})}<strong>Часы</strong></div>{scheduleMonth.employees.map((employee)=>{const entries=days.map((day)=>entryFor(employee.id,day));const hours=entries.reduce((sum,entry)=>sum+(entry?.plannedHours||0),0);return <div className="schedule-grid-row" key={employee.id} style={{gridTemplateColumns:gridColumns}}><strong>{employee.name}</strong>{entries.map((entry,dayIndex)=><span key={dayIndex} className={entry?'schedule-mark':''} title={entry?entry.shiftName+' · '+entry.startTime.slice(0,5)+'–'+entry.endTime.slice(0,5):undefined}>{entry?.shiftCode||entry?.shiftName||'—'}</span>)}<b>{hours} ч.</b></div>})}</div>:<p>Опубликованный график точки пока не загружен.</p>}</section>
    </div>}
