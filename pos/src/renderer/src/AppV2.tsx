...
export function WorkPage({products:_products,data,shiftOpen,onChanged,notify}:{products:Product[];data:WorkplaceData;shiftOpen:boolean;onChanged:()=>Promise<void>;notify:(text:string)=>void}){
  ...
  const [expanded,setExpanded]=useState<Record<string,boolean>>({});const [stockQuery,setStockQuery]=useState('')
  ...
  const gridColumns='minmax(160px,1fr) repeat('+days.length+',minmax(34px,1fr)) 70px'
  ...
  const catalog=data.operationalCatalog||[];const stockProducts=catalog.filter((x)=>x.trackInventory&&['Product','Variant'].includes(x.itemType))
  const query=stockQuery.trim().toLocaleLowerCase();const stockRows=stockProducts.filter((x)=>[x.name,x.id,x.itemCode].some((value)=>value.toLocaleLowerCase().includes(query)))
  return <Page title="Рабочее место" kicker=""><div className="work-tabs"><button className={tab==='schedule'?'active':''} onClick={()=>setTab('schedule')}>График работы</button><button className={tab==='stock'?'active':''} onClick={()=>setTab('stock')}>Товары и склад</button><button className={tab==='cleaner'?'active':''} onClick={()=>setTab('cleaner')}>Уборка{data.cleaner.paymentDueMinor>0&&<b>!</b>}</button></div>
    {tab==='schedule'&&<div className="work-schedule"><section className="work-card schedule-upcoming"><h3>Мои ближайшие 5 смен</h3>{data.myUpcomingShifts.length?data.myUpcomingShifts.map((entry)=><article key={entry.id}><div><b>{new Date(entry.date+'T00:00:00').toLocaleDateString('ru-RU',{weekday:'short',day:'numeric',month:'short'})}</b><small>{entry.shiftCode||entry.shiftName}</small></div><strong>{entry.startTime.slice(0,5)}–{entry.endTime.slice(0,5)} · {entry.plannedHours} ч.</strong></article>):<p>Ближайших опубликованных смен нет.</p>}</section><section className="work-card schedule-month"><header><div><h3>График точки</h3><p>{monthLabel}</p></div><small>Только просмотр</small></header>{days.length>0&&scheduleMonth.employees.length?<div className="schedule-grid"><div className="schedule-grid-row schedule-grid-header" style={{gridTemplateColumns:gridColumns}}><strong>Сотрудник</strong>{days.map((day)=>{const date=new Date(scheduleMonth.month+'-'+String(day).padStart(2,'0')+'T00:00:00');return <span key={day}>{date.toLocaleDateString('ru-RU',{weekday:'short'}).replace('.','')}<b>{day}</b></span>})}<strong>Часы</strong></div>{scheduleMonth.employees.map((employee)=>{const entries=days.map((day)=>entriesFor(employee.id,day));const hours=entries.flat().reduce((sum,entry)=>sum+entry.plannedHours,0);return <div className="schedule-grid-row" key={employee.id} style={{gridTemplateColumns:gridColumns}}><strong>{employee.name}</strong>{entries.map((dayEntries,dayIndex)=>{const label=dayEntries.map((entry)=>entry.shiftCode||entry.shiftName).join(' · ');const title=dayEntries.map((entry)=>entry.shiftName+' · '+entry.startTime.slice(0,5)+'–'+entry.endTime.slice(0,5)).join(' | ');return <span key={dayIndex} className={dayEntries.length?'schedule-mark':''} title={dayEntries.length?title:undefined}>{label||'—'}</span>})}<b>{hours} ч.</b></div>})}</div>:<p>Опубликованный график точки пока не загружен.</p>}</section></div>}
    {tab==='stock'&&<div className="work-grid warehouse-workspace"><div className="work-toolbar warehouse-actions"><div><h2>Товары и склад</h2><p>Остатки и поставки текущей точки.</p></div><button onClick={()=>setWriteOff(true)}>Списать брак</button><button className="primary" onClick={()=>setNeed(true)}>Потребность точки</button></div><section className="work-card delivery-list"><header><h2>Поставки</h2><small>Открытые заказы текущей точки</small></header>{data.deliveries.length?data.deliveries.map((order)=><DeliveryCard key={order.id} order={order} expanded={Boolean(expanded[order.id])} onToggle={()=>setExpanded({...expanded,[order.id]:!expanded[order.id]})} onReceive={()=>setReceiveOrder(order)}/>):<Empty title="Поставок нет" text="Открытые заказы появятся здесь из OS."/ >}</section><section className="work-card stock-list"><header><span>Товар</span><span>Остаток</span><span>Где лежит</span></header><label className="warehouse-search"><span>Поиск</span><input value={stockQuery} onChange={(e)=>setStockQuery(e.target.value)} placeholder="Название, ID или код"/></label>{stockRows.length?stockRows.map((x)=><div key={x.id}><div><b>{x.name}</b><small>{x.itemCode||x.id}</small></div><strong className={(x.stock??0)<=0?'low':''}>{x.stock??'—'} {x.uom}</strong><span>{x.storageAddress||'Адрес ещё не указан'}</span></div>):<p>Складские позиции не найдены.</p>}</section></div>}
    ...
    {need&&<SupplyRequestModal products={catalog} onClose={()=>setNeed(false)} onComplete={async(request)=>{try{await window.raspechatkaPos.createSupplyRequest(request);setNeed(false);await onChanged();notify('Потребность точки отправлена закупщику')}catch(e){notify(e instanceof Error?e.message:String(e))}}}/>}
...
function DeliveryCard({order,expanded,onToggle,onReceive}:{order:DeliveryNotice;expanded:boolean;onToggle:()=>void;onReceive:()=>void}){return <article className="delivery-card"><button className="delivery-summary" onClick={onToggle}><div><b>№ {order.id}</b><strong>{order.supplier}</strong><small>{order.expectedDate?new Date(order.expectedDate+'T00:00:00').toLocaleDateString('ru-RU'):'Дата не назначена'} · {order.status}</small></div><span>{order.comment||order.receivingNote||order.details||'Без комментария'}</span></button>{expanded&&<div className="delivery-detail">{order.items.filter((item)=>item.remainingQuantity>0).map((item)=><div key={item.purchaseOrderItemId}><span>{item.itemName} · {item.itemCode||item.itemId}</span><b>{item.remainingQuantity} {item.uom}</b></div>)}{order.items.some((item)=>item.remainingQuantity>0)&&<button className="primary" onClick={onReceive}>Создать приёмку</button>}</div>}</article>}

function WriteOffModal({products,onClose,onComplete}:{products:OperationalCatalogItem[];onClose:()=>void;onComplete:(request:StockWriteOffRequest)=>Promise<void>}){const [productId,setProductId]=useState(products[0]?.id||'');const [quantity,setQuantity]=useState('1');const [reason,setReason]=useState<StockWriteOffRequest['reason']>('Брак');const [comment,setComment]=useState('');return <div className="modal-backdrop"><div className="payment-modal compact-modal"><header><div><small>СКЛАД ТОЧКИ</small><h2>Списать товар</h2></div><button onClick={onClose}>×</button></header><label className="cash-input"><span>Товар</span><select value={productId} onChange={(e)=>setProductId(e.target.value)}>{products.map((x)=><option key={x.id} value={x.id}>{x.name} · остаток {x.stock??0}</option>)}</select></label><label className="cash-input"><span>Количество</span><input type="number" min="0.001" step="0.001" value={quantity} onChange={(e)=>setQuantity(e.target.value)}/></label><label className="cash-input"><span>Причина</span><select value={reason} onChange={(e)=>setReason(e.target.value as StockWriteOffRequest['reason'])}><option>Брак</option><option>Внутренние нужды</option><option>Обучение</option><option>Другое</option></select></label><label className="cash-input"><span>Комментарий</span><input value={comment} onChange={(e)=>setComment(e.target.value)} placeholder="Что произошло — коротко"/></label><button className="primary confirm" disabled={!productId||Number(quantity)<=0} onClick={()=>onComplete({productId,quantity:Number(quantity),reason,comment})}>Подтвердить списание</button></div></div>}

function SupplyRequestModal({products,onClose,onComplete}:{products:OperationalCatalogItem[];onClose:()=>void;onComplete:(request:SupplyRequestInput)=>Promise<void>}){const [productId,setProductId]=useState('');const [itemName,setItemName]=useState('');const [quantity,setQuantity]=useState('1');const [comment,setComment]=useState('');const select=(id:string)=>{setProductId(id);setItemName(products.find((x)=>x.id===id)?.name||'')};return <div className="modal-backdrop"><div className="payment-modal compact-modal"><header><div><small>ПОТРЕБНОСТЬ ТОЧКИ</small><h2>Что нужно заказать</h2></div><button onClick={onClose}>×</button></header><label className="cash-input"><span>Из каталога</span><select value={productId} onChange={(e)=>select(e.target.value)}><option value="">Другая позиция</option>{products.map((x)=><option key={x.id} value={x.id}>{x.name} · {x.itemCode||x.id}</option>)}</select></label><label className="cash-input"><span>Наименование</span><input value={itemName} onChange={(e)=>setItemName(e.target.value)} placeholder="Например: бумага А4"/></label><label className="cash-input"><span>Количество</span><input type="number" min="0.001" step="0.001" value={quantity} onChange={(e)=>setQuantity(e.target.value)}/></label><label className="cash-input"><span>Комментарий</span><input value={comment} onChange={(e)=>setComment(e.target.value)} placeholder="Срочность или уточнение"/></label><button className="primary confirm" disabled={!itemName.trim()||Number(quantity)<=0} onClick={()=>onComplete({productId:productId||undefined,itemName:itemName.trim(),quantity:Number(quantity),comment})}>Отправить закупщику</button></div></div>}

function ReceiveModal({order,onClose,onComplete}:{order:DeliveryNotice;onClose:()=>void;onComplete:(request:StockReceiptRequest)=>Promise<void>}){const [lines,setLines]=useState(()=>order.items.filter((item)=>item.remainingQuantity>0).map((item)=>({purchaseOrderItemId:item.purchaseOrderItemId,itemName:item.itemName,uom:item.uom,remainingQuantity:item.remainingQuantity,quantity:item.remainingQuantity})));const update=(id:string,quantity:number)=>setLines(lines.map((line)=>line.purchaseOrderItemId===id?{...line,quantity}:line));return <div className="modal-backdrop"><div className="payment-modal compact-modal"><header><div><small>ПРИЁМКА ПОСТАВКИ</small><h2>№ {order.id}</h2></div><button onClick={onClose}>×</button></header>{lines.length?lines.map((line)=><div className="receive-line" key={line.purchaseOrderItemId}><div><b>{line.itemName}</b><small>Остаток: {line.remainingQuantity} {line.uom}</small></div><input type="number" min="0" max={line.remainingQuantity} step="0.001" value={line.quantity} onChange={(e)=>update(line.purchaseOrderItemId,Number(e.target.value))}/><button title="Убрать строку" onClick={()=>setLines(lines.filter((x)=>x.purchaseOrderItemId!==line.purchaseOrderItemId))}>×</button></div>):<p>Нет строк для приёмки.</p>}<button className="primary confirm" disabled={!lines.some((line)=>line.quantity>0)} onClick={()=>onComplete({purchaseOrderId:order.id,lines:lines.filter((line)=>line.quantity>0).map((line)=>({purchaseOrderItemId:line.purchaseOrderItemId,quantity:Math.min(line.quantity,line.remainingQuantity)}))})}>Подтвердить приёмку</button></div></div>}
