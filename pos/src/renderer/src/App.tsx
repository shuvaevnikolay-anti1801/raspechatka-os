...
export default function App(){
  ...
  const refresh=async()=>{
    const result=await Promise.all([
      ...
      window.raspechatkaPos.getLastCashCount(),window.raspechatkaPos.getPosLifecycle()
    ])
    ...
    setWorkplace(result[9]);setOrders(result[10]);setLastCashCount(result[11]);setLifecycle(result[12])
  }
  ...
  if(!boot||!lifecycle)return <div className="loading"><i/>Запускаем кассу…</div>
