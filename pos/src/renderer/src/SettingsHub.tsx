...
export default function SettingsHub(){
  ...
  const [devices,setDevices]=useState<DeviceStatuses>(null as unknown as DeviceStatuses)
    ...
    {open&&<div className="settings-hub">
      ...
      <div className="settings-hub-body">
        ...
        <section className="settings-section"><div className="section-heading"><div><h2>ККТ АТОЛ</h2><p>АТОЛ 1Ф через локальный Web Server Драйвера ККТ 10.</p></div><label className="toggle"><input type="checkbox" checked={atol.enabled} onChange={(e)=>setAtol({...atol,enabled:e.target.checked})}/> Использовать АТОЛ</label></div>
          <div className="settings-form-grid"><label><span>Статус</span><strong>{devices.fiscal.ready?'🟢 Подключено':atol.enabled?'🟠 Требуется проверка':'Выключено'}</strong></label><label><span>ККТ</span><strong>АТОЛ 1Ф</strong></label><label><span>Оператор</span><strong>Текущий кассир</strong></label><label><span>СНО</span><select value={atol.taxationType} onChange={(e)=>setAtol({...atol,taxationType:e.target.value})}><option value="patent">Патент</option><option value="usnIncome">УСН доход</option><option value="usnIncomeOutcome">УСН доход − расход</option><option value="osn">ОСН</option></select></label><label><span>НДС</span><select value={atol.taxType} onChange={(e)=>setAtol({...atol,taxType:e.target.value})}><option value="none">Без НДС</option><option value="vat0">0%</option><option value="vat5">5%</option><option value="vat7">7%</option><option value="vat10">10%</option><option value="vat20">20%</option><option value="vat22">22%</option></select></label></div><button className="primary section-action" onClick={()=>void saveAtol()}>Сохранить и проверить АТОЛ</button>
        </section>
        ...
        <section className="settings-section"><div className="section-heading"><div><h2>Эквайринг INPAS / PAX</h2><p>Интеграция через официальный DC Console.exe.</p></div><label className="toggle"><input type="checkbox" checked={inpas.enabled} onChange={(e)=>setInpas({...inpas,enabled:e.target.checked})}/> Использовать INPAS</label></div>
          <div className="settings-form-grid"><label className="wide"><span>Путь к DC Console.exe</span><input value={inpas.executablePath} onChange={(e)=>setInpas({...inpas,executablePath:e.target.value})} placeholder="Можно оставить пустым для автопоиска"/></label><label><span>ID терминала</span><input value={inpas.terminalId} onChange={(e)=>setInpas({...inpas,terminalId:e.target.value})}/></label><label><span>Код валюты</span><input value={inpas.currencyCode} inputMode="numeric" onChange={(e)=>setInpas({...inpas,currencyCode:e.target.value})}/></label><label><span>Таймаут, сек.</span><input type="number" min="30" max="3600" value={Math.round(inpas.timeoutMs/1000)} onChange={(e)=>setInpas({...inpas,timeoutMs:Number(e.target.value)*1000})}/></label></div><div className="settings-actions"><button className="primary" onClick={()=>void saveInpas()}>Сохранить INPAS</button><button onClick={()=>void terminal('test')}>Проверить связь</button><button onClick={()=>void terminal('reconcile')}>Сверка итогов</button></div>
        </section>
        ...
        <section className="settings-section"><div className="section-heading"><div><h2>Диагностика</h2><p>Последние технические события приложения.</p></div><button onClick={()=>void refresh()}>Обновить</button></div><div className="settings-log">{diagnostics.length?diagnostics.slice(0,40).map((x)=><article key={x.id} className={x.level}><time>{new Date(x.createdAt).toLocaleString('ru-RU')}</time><div><b>{x.source} · {x.eventType}</b><span>{x.message}</span></div></article>):<div className="settings-ok">Диагностических событий пока нет.</div>}</div></section>
