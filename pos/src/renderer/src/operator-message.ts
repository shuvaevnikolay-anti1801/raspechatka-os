...
export function operatorError(error:unknown,domain:Domain='general'):string{
  ...
  const messages:Record<Exclude<Domain,'payment'|'fiscal'|'terminal'|'shift'>,string>={
    general:'Итог действия не подтверждён. Проверьте состояние перед новым действием.',
    sync:'Итог обновления не подтверждён. Работа на кассе продолжается локально; проверьте очередь синхронизации.',
    settings:'Не удалось подтвердить состояние настроек. Проверьте подключение и диагностику.',
    orders:'Итог действия с заказом не подтверждён. Проверьте заказ и очередь перед новым действием.',
    work:'Итог действия не подтверждён. Проверьте данные точки и очередь перед новым действием.',
