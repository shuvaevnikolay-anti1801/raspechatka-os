...
class TestPointCleaningReferences(TestCase):
	def test_defaults_and_custom_values(self):
		for payload, expected in [
			...
			(
				{
					"business_entity": "ENTITY-A",
					"cleaning_payout_amount": 3250.5,
			...
			point = _Point()
			with (
				patch.object(references, "require_access") as access,
				patch.object(references, "_validate_payload_scope"),
				...
				self.assertEqual(
					(point.get("cleaning_payout_amount"), point.get("cleaning_every_n_visits")), expected
				)

	def test_invalid_values_are_rejected(self):
		for field, value in [
			("cleaning_payout_amount", 0),
			("cleaning_payout_amount", -1),
			("cleaning_payout_amount", "bad"),
			("cleaning_every_n_visits", 0),
			("cleaning_every_n_visits", 1.5),
			("cleaning_every_n_visits", "bad"),
		]:
			with (
				self.subTest(field=field, value=value),
				patch.object(references, "require_access"),
				patch.object(references, "_validate_payload_scope"),
				patch.object(references.frappe, "new_doc", return_value=_Point()),
			):
	...
	def test_foreign_point_is_denied_before_update(self):
		with (
			...
			patch.object(references.frappe.db, "exists", return_value=False),
			patch.object(references.frappe, "get_doc") as get_doc,
		):
	...
	def test_detail_defaults_for_existing_unset_point(self):
		...
		with (
			patch.object(references, "require_access") as access,
			patch.object(references, "_scope_filters", return_value={}),
			patch.object(references.frappe.db, "exists", return_value=True),
			patch.object(references.frappe, "get_doc", return_value=point),
		):
...
class TestPointCleaningBootstrap(TestCase):
	def test_bootstrap_carries_point_cleaning_config(self):
		...
		with (
			...
			patch.object(pos_v2.base_pos, "_workplace", return_value=workplace),
			patch.object(pos_v2.base_pos, "_point_employees", return_value=[]),
			patch.object(pos_v2.base_pos, "_bootstrap_employee", return_value=None),
			patch.object(pos_v2.base_pos, "_touch"),
			patch.object(pos_v2, "_products", return_value=[]),
			patch.object(pos_v2, "_customers", return_value=[]),
			patch.object(pos_v2, "_rules", return_value={}),
			patch.object(pos_v2, "_upsell_rules", return_value=[]),
			patch.object(pos_v2, "_receipt_mirror", return_value=[]),
			patch.object(pos_v2.legacy_pos, "_get_workplace_data", return_value={}),
