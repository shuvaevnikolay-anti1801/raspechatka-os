...
class TestCleanerPayoutEvidence(TestCase):
	def setUp(self):
		self.connection = SimpleNamespace(business_point="POINT-A")
		self.payload = {
			"id": "CASH-1", "amountMinor": 325050, "payoutAmountMinor": 325050,
			"cleaningPayoutId": "PAYOUT-1", "cleaningCycleId": "CYCLE-1",
			"everyNVisits": 3, "cleaningVisitEventIds": ["VISIT-1", "VISIT-2", "VISIT-3"],
		}
		self.movement = SimpleNamespace(
			name="CASH-MOVEMENT-1", business_point="POINT-A", movement_type="Withdrawal",
			amount=3250.5, cashier="EMP-A", docstatus=1,
			cleaning_payout_id="PAYOUT-1", cleaning_cycle_id="CYCLE-1",
		)
		self.visits = [
			SimpleNamespace(
				name=f"CLEAN-{index}", source_pos_event=f"VISIT-{index}",
				paid=0, payment_pos_event=None, cleaning_cycle_id="CYCLE-1", cleaning_payout_id=None,
			)
	...
	def test_exact_cycle_and_cash_evidence_mark_only_listed_visits(self):
		...
		with patch.object(pos_v2.frappe.db, "sql", return_value=[("POINT-A",)]), patch.object(
			pos_v2.frappe.db, "get_value", return_value=self.movement
		), patch.object(pos_v2.frappe, "get_all", return_value=self.visits) as get_all, patch.object(
			pos_v2.frappe, "get_doc", side_effect=docs
			...
			self.assertEqual(get_all.call_args.kwargs["filters"]["source_pos_event"], ["in", self.payload["cleaningVisitEventIds"]])
			for doc in docs:
	...
	def test_replay_with_same_reference_is_noop(self):
		...
		with patch.object(pos_v2.frappe.db, "sql", return_value=[("POINT-A",)]), patch.object(
			pos_v2.frappe.db, "get_value", return_value=self.movement
		), patch.object(pos_v2.frappe, "get_all", return_value=paid), patch.object(
			pos_v2.frappe, "get_doc"
		) as get_doc:
			pos_v2._ingest_cleaner_payment(self.payload, self.connection, "EMP-A")
			get_doc.assert_not_called()
	...
	def test_missing_visit_foreign_cycle_or_wrong_cash_evidence_never_marks_paid(self):
		with patch.object(pos_v2.frappe.db, "sql", return_value=[("POINT-A",)]), patch.object(
			pos_v2.frappe.db, "get_value", return_value=self.movement
		), patch.object(pos_v2.frappe, "get_all", return_value=self.visits[:2]), patch.object(
			pos_v2.frappe, "get_doc"
		) as get_doc:
			with self.assertRaises(frappe.ValidationError):
				pos_v2._ingest_cleaner_payment(self.payload, self.connection, "EMP-A")
		...
		with patch.object(pos_v2.frappe.db, "sql", return_value=[("POINT-A",)]), patch.object(
			pos_v2.frappe.db, "get_value", return_value=wrong
		), patch.object(pos_v2.frappe, "get_doc") as get_doc:
			with self.assertRaises(frappe.ValidationError):
				pos_v2._ingest_cleaner_payment(self.payload, self.connection, "EMP-A")
			get_doc.assert_not_called()
	...
	def test_cash_dispatch_uses_standard_movement_then_reconciles_visits(self):
		...
		with patch.object(pos_v2.base_pos, "_authenticate", return_value=connection), patch.object(
			...
			pos_v2, "_normalize_v2_payload", side_effect=lambda value: value
		), patch.object(pos_v2.base_pos, "_cash", return_value={"external_id": "CASH-1"}) as cash, patch.object(
			pos_v2.sales_api, "_ingest_cash"
		) as ingest, patch.object(pos_v2, "_ingest_cleaner_payment") as reconcile, patch.object(
			pos_v2.base_pos, "_touch"
		), patch.object(pos_v2.frappe.db, "savepoint"):
			result = pos_v2.push_events("device", "token", events=[event, event])
