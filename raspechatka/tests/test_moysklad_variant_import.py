...
class TestMoySkladVariantImportContract(TestCase):
	...
	def test_entity_variants_are_upserted_as_variants(self):
		variant_loop = self.source.split("for row in variants:", 1)[1].split("for row in bundles:", 1)[0]
	...
	def test_characteristics_are_canonical_variant_values_before_save(self):
		upsert = self.source.split("def _upsert_item(", 1)[1].split("def _load_bundle_details", 1)[0]
	...
	def test_parent_variant_flag_uses_linked_active_variants(self):
		sync = self.source.split("def _sync_catalog", 1)[1].split("def _sync_groups", 1)[0]
	...
	def test_variant_values_do_not_come_from_item_name(self):
		upsert = self.source.split("def _upsert_item(", 1)[1].split("def _load_bundle_details", 1)[0]
		values_block = upsert.split('if item_type == "Variant":', 1)[1].split('if item_type == "Bundle":', 1)[
			0
		]
