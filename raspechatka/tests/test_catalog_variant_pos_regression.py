...
class TestCatalogVariantPosRegressionContract(TestCase):
	def setUp(self):
		...
		self.catalog_page = (root.parent / "frontend/src/pages/CatalogPage.vue").read_text(encoding="utf-8")
		self.repair_patch = (root / "patches/v1_0/repair_moysklad_variants.py").read_text(encoding="utf-8")

	def test_catalog_list_exposes_variant_parent_and_inherited_group(self):
		catalog_list = self.frontend_api.split("def get_catalog_items", 1)[1].split(
			"def get_catalog_filters", 1
		)[0]
	...
	def test_parent_card_loads_linked_variants_without_new_model(self):
		item_api = self.frontend_api.split("def get_catalog_item", 1)[1].split(
			"def search_bundle_components", 1
		)[0]
	...
	def test_pos_excludes_variant_parent_but_keeps_assorted_variant(self):
		products = self.pos_api.split("def _get_products", 1)[1].split("def _get_customers", 1)[0]
		self.assertIn("if not item or item.has_variants:", products)
		self.assertIn('"has_variants"', self.frontend_api)
		self.assertIn('"variant_of": parent_name', self.repair_patch)
		self.assertIn('"item_type": "Variant"', self.repair_patch)
		self.assertIn('"active": 1', self.repair_patch)
