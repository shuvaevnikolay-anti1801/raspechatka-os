...
class TestPosUpsellContract(TestCase):
	def setUp(self):
		...
		self.rule = (root / "raspechatka_os/doctype/pos_upsell_rule/pos_upsell_rule.json").read_text(
			encoding="utf-8"
		)
		self.candidate = (
			root / "raspechatka_os/doctype/pos_upsell_candidate/pos_upsell_candidate.json"
	...
	def test_snapshot_reconcile_validates_then_rolls_back(self):
		self.assertLess(
			self.helper.index("rules = _validate_snapshot(snapshot)"),
			self.helper.index("frappe.db.savepoint"),
		)
	...
	def test_api_has_network_contract_and_global_write_guard(self):
		...
		self.assertIn(
			'@access_contract(area="page.sales.integration", action="read", scope="network")', self.sales
		)
		self.assertIn(
			'@access_contract(area="page.sales.integration", action="write", scope="network")', self.sales
		)
