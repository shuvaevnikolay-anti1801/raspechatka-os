...
class TestCleanerCashMovementContract(TestCase):
	def test_standard_cash_ingest_persists_cleaner_reference_and_expense_purpose(self):
		...
		with (
			patch.object(sales.frappe.db, "exists", return_value=False),
			patch.object(sales, "_shift_name", return_value="SHIFT-CANONICAL"),
			patch.object(sales, "_set_doc_scope"),
			patch.object(sales.frappe, "new_doc", return_value=doc),
		):
