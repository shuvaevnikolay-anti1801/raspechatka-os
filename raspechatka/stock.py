...
def get_item(item_name):
	...
	if not item or item.item_type not in {"Product", "Variant"} or not item.track_inventory or not item.active:
...
def make_ledger_entry(document, row, quantity, rate, amount, reversal=False, valuation_source=None):
	...
	if qty_after < -EPSILON or location_qty_after < -EPSILON:
		item_label = frappe.db.get_value("Catalog Item", row.item, "item_name") or row.item
		available = min(flt(warehouse_balance["qty"]), flt(location_balance["qty"]))
		frappe.throw(
			_("Недостаточно остатка товара {0}: доступно {1}.").format(item_label, max(available, 0))
		)
