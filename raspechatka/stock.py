...
def validate_location(storage_location, warehouse):
	if (
		storage_location
		and frappe.db.get_value("Storage Location", storage_location, "warehouse") != warehouse
	):
...
def make_ledger_entry(document, row, quantity, rate, amount, reversal=False, valuation_source=None):
	...
	entry.valuation_source = valuation_source or _default_valuation_source(quantity, reversal)
...
def write_operational_balance(item, warehouse, quantity, stock_value, last_movement_at, last_ledger_entry):
	...
	reserved = flt(frappe.db.get_value("Stock Balance", key, "reserved_qty") if existing else 0)
	values = {
		"item": item,
		"warehouse": warehouse,
