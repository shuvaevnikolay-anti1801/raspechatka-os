...
def _valid_inn12(seed):
	...
	check_11 = sum(value * weight for value, weight in zip(digits, (7, 2, 4, 10, 3, 5, 9, 4, 6, 8))) % 11 % 10
	first_11 = digits + [check_11]
	check_12 = sum(value * weight for value, weight in zip(first_11, (3, 7, 2, 4, 10, 3, 5, 9, 4, 6, 8))) % 11 % 10
...
class IntegrationTestBusinessPoint(IntegrationTestCase):
	def test_working_hours_survive_save_and_reopen(self):
		...
		point = frappe.get_doc({
			...
			"working_hours": [{
				"weekday": "Понедельник", "is_working": 1,
				"opens_at": "08:15", "closes_at": "19:45",
			}],
		}).insert(ignore_permissions=True)
