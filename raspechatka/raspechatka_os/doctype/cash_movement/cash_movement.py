...
class CashMovement(Document):
	...
	def validate(self):
		...
		shift = frappe.db.get_value(
			"Sales Shift",
			self.shift,
			["business_entity", "business_point", "cashier", "opened_at", "closed_at"],
			as_dict=True,
		)
		if not shift or (shift.business_entity, shift.business_point) != (
	...
	def on_cancel(self):
		...
		transaction_name = frappe.db.get_value(
			"Finance Transaction", {"cash_movement": self.name, "docstatus": 1}, "name"
		)
		if transaction_name:
