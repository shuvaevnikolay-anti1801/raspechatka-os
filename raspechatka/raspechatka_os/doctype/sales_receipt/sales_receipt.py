...
class SalesReceipt(Document):
    ...
    def validate(self):
        ...
        if not shift or (
            ...
            frappe.throw(
                _("Чек должен относиться к ИП, точке и складу выбранной смены")
            )
        ...
        if (
            self.receipt_type == "Return"
            and not self.original_receipt
            and not mirror_only
        ):
        ...
        if (
            ...
            and frappe.db.get_value(
                "Sales Receipt", self.original_receipt, "receipt_type"
            )
            != "Sale"
        ):
        ...
        if (
            self.original_receipt
            and frappe.db.get_value(
                "Sales Receipt", self.original_receipt, "business_point"
            )
            != self.business_point
        ):
        ...
        for row in self.items:
            ...
            if flt(row.discount_amount) < 0 or flt(row.discount_amount) > flt(
                row.gross_amount
            ):
                frappe.throw(_("Некорректная скидка в строке {0}").format(row.idx))
            actual_discount_percent = (
                flt(row.discount_amount) / flt(row.gross_amount) * 100
            ...
            if (
                ...
                and (
                    ...
                    or actual_discount_percent
                    > flt(point_rules.max_discount_percent) + 0.001
                )
            ):
                frappe.throw(
                    _("Скидка в строке {0} превышает разрешённую для точки").format(
                        row.idx
                    )
                )
        ...
        self.profit_amount = (-1 if self.receipt_type == "Return" else 1) * (
            flt(self.total_amount) - cost
        )
    ...
    def _validate_return_quantities(self):
        ...
        for row in self.items:
            ...
            if requested[row.item] > available.get(row.item, 0) + 0.000001:
                frappe.throw(
                    _(
                        "Возвращаемое количество товара {0} превышает остаток исходной продажи"
                    ).format(row.item_name)
                )
    ...
    def _create_stock_entries(self, reversal):
        for row in self.items:
            ...
            entry.posting_datetime = (
                now_datetime() if reversal else self.posting_datetime
            )
    ...
    def _create_profitability(self, reversal):
        ...
        for row in self.items:
            ...
            entry.quantity = (
                factor
                * flt(row.quantity)
                * (-1 if self.receipt_type == "Return" else 1)
            )
            if self.receipt_type == "Sale":
