...
class SalesShift(Document):
	...
	def validate(self):
		point = frappe.db.get_value(
			"Business Point", self.business_point, ["business_entity", "active"], as_dict=True
		)
		...
		if self.cashier:
			employee = frappe.db.get_value(
				"Employee", self.cashier, ["business_entity", "active"], as_dict=True
			)
			assigned = frappe.db.exists(
				"Employee Point Assignment", {"parent": self.cashier, "business_point": self.business_point}
			)
			if (
				not employee
				or not employee.active
				or employee.business_entity != self.business_entity
				or not assigned
			):
		...
		other = frappe.db.exists(
			"Sales Shift",
			{"business_point": self.business_point, "status": "Open", "name": ["!=", self.name or ""]},
		)
