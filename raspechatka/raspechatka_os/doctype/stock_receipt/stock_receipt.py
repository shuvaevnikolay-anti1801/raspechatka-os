...
class StockReceipt(Document):
    ...
    def before_cancel(self):
        for row in self.items:
            ...
            available = get_balance(row.item, self.warehouse, row.storage_location)[
                "qty"
            ]
            if not item.allow_negative_stock and available < flt(row.quantity):
                frappe.throw(
                    _(
                        "Нельзя отменить приёмку: часть товара {0} уже выбыла со склада."
    ...
    def _validate_header(self):
        ...
        if (
            frappe.db.get_value(
                "Business Point", self.business_point, "business_entity"
            )
            != self.business_entity
        ):
            frappe.throw(_("Точка продаж не относится к выбранному юридическому лицу."))
        if (
            frappe.db.get_value("Catalog Warehouse", self.warehouse, "business_point")
            != self.business_point
        ):
        ...
        if self.purchase_order:
            ...
            if (
                ...
                frappe.throw(
                    _(
                        "Поставщик, ИП, точка и склад должны совпадать со связанным заказом."
                    )
                )
    ...
    def _validate_items(self):
        ...
        for row in self.items:
            ...
            if flt(row.rate) <= 0:
                frappe.throw(
                    _("Закупочная цена обязательна и должна быть больше нуля.")
                )
            ...
            if row.storage_location:
                ...
                if location_warehouse != self.warehouse:
                    frappe.throw(
                        _("Место хранения должно относиться к складу документа.")
                    )
            key = (row.item, row.storage_location or "")
            if key in seen:
                frappe.throw(_("Одинаковый товар и место хранения указаны дважды."))
            seen.add(key)
            locations_by_item.setdefault(row.item, set()).add(
                row.storage_location or ""
            )
            if (
                "" in locations_by_item[row.item]
                and len(locations_by_item[row.item]) > 1
            ):
                frappe.throw(
                    _(
                        "Нельзя одновременно принять товар без адреса и на отдельное место хранения."
                    )
                )
            ...
            if self.purchase_order:
                if not row.purchase_order_item:
                    frappe.throw(
                        _(
                            "Каждая строка приёмки по заказу должна быть связана со строкой заказа."
                        )
                    )
                ...
                if (
                    not order_row
                    or order_row.parent != self.purchase_order
                    or order_row.item != row.item
                ):
                    frappe.throw(
                        _(
                            "Строка приёмки не соответствует связанному заказу поставщику."
                        )
                    )
                ...
                if already_received + flt(row.quantity) > flt(order_row.quantity):
                    frappe.throw(
                        _(
                            "Количество приёмки превышает остаток по заказу для товара {0}."
                        ).format(item.item_name)
                    )
    ...
    def _make_ledger_entries(self, reversal=False):
        for row in self.items:
            make_ledger_entry(
                ...
                flt(row.amount),
                reversal=reversal,
                valuation_source="Purchase receipt"
                if self.receipt_type == "Приёмка"
                else "Stock receipt",
            )
