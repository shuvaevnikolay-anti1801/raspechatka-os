...
def log_cashier_action(document, action_type, external_id=None, details=None, metric_value=1, gift_tier=None):
	...
	action.action_datetime = (
		getattr(document, "posting_datetime", None) or getattr(document, "opened_at", None) or now_datetime()
	)
...
def update_shift_totals(shift_name):
	...
	payments = frappe.get_all(
		...
		filters={
			"parent": [
				"in",
				frappe.get_all("Sales Receipt", filters={"shift": shift_name, "docstatus": 1}, pluck="name")
				or ["__none__"],
			]
		},
	...
	receipt_types = {
		row.name: row.receipt_type
		for row in frappe.get_all(
			"Sales Receipt",
			filters={"shift": shift_name, "docstatus": 1},
			fields=["name", "receipt_type"],
			limit_page_length=100000,
		)
	}
	...
	actions = frappe.get_all(
		...
		filters={
			"shift": shift_name,
			"action_type": ["in", ["REVIEW_RECEIVED", "CLUB_REGISTRATION", "GIFT_ORDER"]],
		},
		fields=["action_type", "metric_value", "gift_tier"],
		limit_page_length=100000,
	)
	...
	values = {
		...
		"gross_sales": sum(flt(r.total_amount) for r in sales),
		"returns_total": sum(flt(r.total_amount) for r in returns),
		"sales_before_discount": sum(flt(r.gross_amount) for r in sales),
		"discounts_total": sum(flt(r.discount_amount) for r in sales),
		"review_discounts": sum(flt(r.review_discount_amount) for r in sales),
		"other_discounts": sum(flt(r.other_discount_amount) for r in sales),
		"discounted_receipt_count": sum(1 for r in sales if flt(r.discount_amount) > 0),
		"cash_sales": channels["Cash"],
		"card_sales": channels["Card"],
		"qr_sales": channels["QR"],
		"reviews_count": metrics["REVIEW_RECEIVED"],
		"club_registrations": metrics["CLUB_REGISTRATION"],
		"gift_orders": metrics["GIFT_ORDER"],
		"gift_orders_1": gifts["GIFT_1"],
		"gift_orders_2": gifts["GIFT_2"],
		"gift_orders_3": gifts["GIFT_3"],
	}
