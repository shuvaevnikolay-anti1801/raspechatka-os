...
def _shift_actions(doc):
    ...
    for filters in (
        ...
        for row in frappe.get_all(
            "Cashier Action", filters=filters,
            fields=["name", "action_type", "shift", "business_entity", "business_point",
...
def _sales_shift(doc, *, execute=False):
    ...
    result = {
        "strategy": "hard_delete", "deleted": bool(execute), "can_delete": True,
    ...
    if execute:
        for name in technical:
            frappe.delete_doc("Cashier Action", name, ignore_permissions=True)
...
def _employee(doc, *, execute=False):
    ...
    if open_shifts:
        blocked = _dependency_result({"Sales Shift": open_shifts},
                                     _("Сначала закройте или удалите открытую смену кассира"))
    ...
    result = {
        ...
        "message": _("История сохранится; сотрудник будет деактивирован")
                   if strategy == "deactivate" else _("Неиспользуемый сотрудник будет удалён"),
        "dependencies": {doctype: {"count": len(names), "names": names}
    ...
    if execute:
        if strategy == "hard_delete":
            ...
            result["affected"] = [{"doctype": "Employee", "name": doc.name,
                                   "active": 0, "pos_access_enabled": 0}]
    return result
