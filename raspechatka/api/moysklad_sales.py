...
def discover_sales_sources():
    ...
    settings = frappe.get_single("MoySklad Settings")
    payload = _request(
        settings, "entity/retailstore", params={"limit": 100, "offset": 0}
    )
    return {
        "stores": [
            {
...
def save_sales_sync_settings(data):
    ...
    settings.sales_sync_interval_minutes = str(
        max(5, min(cint(data.get("interval_minutes") or 5), 60))
    )
    ...
    for mapping in mappings:
        ...
        frappe.db.set_value(
            ...
            point_name,
            {
                ...
                "moysklad_retail_store_name": mapping.get("source_name")
                if source_id
                else None,
            },
        )
...
def enqueue_sales_sync(full=False):
    ...
    if not frappe.db.exists(
        "Business Point", {"moysklad_retail_store_id": ["!=", ""], "active": 1}
    ):
...
def run_sales_sync(full=False):
    ...
    try:
        cursor = (
            None
            if stats["mode"] == "full"
            else get_datetime(settings.sales_sync_cursor)
        )
...
def _iter_rows(settings, endpoint, cursor=None):
    ...
    if cursor:
        ...
        filter_value = (
            f"updated>={cursor.strftime('%Y-%m-%d %H:%M:%S')};moment>={from_moment}"
        )
    else:
...
def _upsert_shift(row, context):
    ...
    doc.business_entity = frappe.db.get_value(
        "Business Point", point, "business_entity"
    )
...
def _upsert_receipt(row, context, is_return=False):
    ...
    for position in positions:
        ...
        item = frappe.db.get_value(
            "Catalog Item", {"moysklad_id": moysklad_item_id}, "name"
        )
        if not item:
        ...
        items.append(
            {
                ...
                "discount_amount": round(
                    quantity * unit_price * discount_percent / 100, 2
                ),
            }
        )
    if not items:
    ...
    doc = (
        frappe.get_doc("Sales Receipt", name)
        if name
        else frappe.new_doc("Sales Receipt")
    )
...
def _payments(row, expected_total):
    ...
    cash = _money(
        row.get("cashSum")
        if row.get("cashSum") is not None
        else container.get("cashSum")
    )
    noncash = _money(
        row.get("noCashSum")
        if row.get("noCashSum") is not None
        else container.get("cardSum")
    )
    qr = _money(
        row.get("qrSum") if row.get("qrSum") is not None else container.get("qrSum")
    )
    ...
    return [
        {"payment_channel": channel, "amount": amount}
        for channel, amount in values
        if amount > 0
    ]
...
def _mapped_point(row, context):
    ...
    if not store_id and row.get("retailShift"):
        ...
        shift = (
            _request(context["settings"], f"entity/retailshift/{shift_id}")
            if shift_id
            else {}
        )
...
def _employee(reference, point, context):
    ...
    if source_id not in context["employee_cache"]:
        ...
        name = (
            frappe.db.get_value("Employee", {"email": email}, "name") if email else None
        )
    ...
    if name and frappe.db.exists(
        "Employee Point Assignment", {"parent": name, "business_point": point}
    ):
        return name
    return None
