...
from raspechatka.pos_upsell import get_pos_upsell_config, save_pos_upsell_rules
...
def get_points_overview(from_date=None, to_date=None, business_entity=None, business_point=None):
	...
	points = frappe.get_all(
		...
		fields=["name", "point_name", "city", "business_entity"],
		order_by="point_name asc",
	)
	result = []
	for point in points:
		...
		shifts = frappe.get_all(
			"Sales Shift",
			filters={"name": ["in", shift_names or ["__none__"]]},
			fields=["net_sales", "receipt_count", "discounts_total", "returns_total", "expected_cash"],
			limit_page_length=100000,
		)
		open_shift = frappe.db.get_value(
			"Sales Shift",
			{"business_point": point.name, "status": "Open"},
			["name", "opened_at", "cashier", "expected_cash"],
			as_dict=True,
		)
		connection = frappe.db.get_value(
			"POS Connection",
			{"business_point": point.name},
			["status", "last_sync_at", "last_seen_at", "app_version", "enabled"],
			as_dict=True,
		)
		if connection:
			connection["status"] = _live_connection_status(connection)
		revenue = sum(flt(row.net_sales) for row in shifts)
		receipts = sum(cint(row.receipt_count) for row in shifts)
		latest_cash = frappe.db.get_value(
			"Sales Shift",
			{"business_point": point.name, "status": ["!=", "Cancelled"]},
			"expected_cash",
		...
		result.append(
			{
				**point,
				...
				"average_check": revenue / receipts if receipts else 0,
				"discounts": sum(flt(row.discounts_total) for row in shifts),
				"returns": sum(flt(row.returns_total) for row in shifts),
				"cash_balance": flt(open_shift.expected_cash) if open_shift else flt(latest_cash),
	...
	return {
		...
		"totals": {
			...
			"discounts": sum(r["discounts"] for r in result),
			"returns": sum(r["returns"] for r in result),
		},
	}
...
def get_shifts(
	...
	or_filters = (
		{"name": ["like", f"%{search.strip()}%"], "external_id": ["like", f"%{search.strip()}%"]}
		if search
		else None
	)
	rows = frappe.get_all(
		...
		order_by="opened_at desc",
		limit_page_length=min(max(cint(limit_page_length), 1), 10000),
	)
	return {
		"rows": rows,
		"totals": {
			"net_sales": sum(flt(r.net_sales) for r in rows),
			"receipts": sum(cint(r.receipt_count) for r in rows),
			"returns": sum(flt(r.returns_total) for r in rows),
			"discounts": sum(flt(r.discounts_total) for r in rows),
		},
	}
...
def get_receipts(
	...
	return {
		"rows": rows,
		"totals": {
			"amount": sum(flt(r.total_amount) for r in rows),
			"discount": sum(flt(r.discount_amount) for r in rows),
			"profit": sum(flt(r.profit_amount) for r in rows),
		},
	}
...
def get_cash_movements(
	...
	or_filters = (
		{"name": ["like", f"%{search.strip()}%"], "reason": ["like", f"%{search.strip()}%"]}
		if search
		else None
	)
	rows = frappe.get_all(
	...
	return {
		"rows": rows,
		"totals": {
			"deposits": sum(flt(r.amount) for r in rows if r.movement_type == "Deposit"),
			"withdrawals": sum(flt(r.amount) for r in rows if r.movement_type == "Withdrawal"),
		},
	}
...
def get_cashier_actions(
	...
	rows = frappe.get_all(
		...
		order_by="action_datetime desc",
		limit_page_length=min(max(cint(limit_page_length), 1), 1000),
	)
	return {"rows": rows}
...
@frappe.whitelist(methods=["POST"])
@access_contract(area="page.sales.integration", action="write", scope="network")
def save_pos_upsell_rules_api(data):
	require_access("page.sales.integration", "write")
	if not get_scope()["global"]:
		frappe.throw(_("Общие настройки продаж доступны только администратору сети"), frappe.PermissionError)
	data = frappe.parse_json(data) if isinstance(data, str) else data
	return save_pos_upsell_rules(data)
...
def push_batch(device_id, token, payload):
	...
	connection.app_version = data.get("app_version") or connection.app_version
	result = {"created": 0, "duplicates": 0, "errors": []}
	try:
		for row in data.get("shifts") or []:
			_ingest_shift(row, connection, result)
		for row in data.get("receipts") or []:
			_ingest_receipt(row, connection, result)
		for row in data.get("cash_movements") or []:
			_ingest_cash(row, connection, result)
		for row in data.get("actions") or []:
			_ingest_action(row, connection, result)
		for row in data.get("shifts") or []:
			if row.get("status") == "Closed":
				_ingest_shift(row, connection, result, update_existing=True)
...
def _ingest_shift(row, connection, result, update_existing=False):
	...
	if not name:
		...
		result["created"] += 1
	if doc.status == "Closed" and doc.closed_at:
		log_cashier_action(doc, "CLOSE_SHIFT", f"{external_id}:close")
...
def _ingest_receipt(row, connection, result):
	...
	doc.external_id = external_id
	doc.receipt_type = row.get("receipt_type") or "Sale"
	doc.naming_series = "RETURN-.YYYY.-.#####" if doc.receipt_type == "Return" else "SALE-.YYYY.-.#####"
	...
	if doc.receipt_type == "Return" and row.get("original_external_id"):
		doc.original_receipt = frappe.db.get_value(
			"Sales Receipt", {"external_id": row.get("original_external_id")}, "name"
		)
...
def _ingest_action(row, connection, result):
	external_id = _required(row, "external_id")
	if frappe.db.exists("Cashier Action", {"external_id": external_id}):
		result["duplicates"] += 1
		return
...
def _required(row, key):
	value = row.get(key)
	if value in (None, ""):
		frappe.throw(_("В пакете кассы отсутствует поле {0}").format(key))
	return value
...
def _shift_name(external_id, point):
	...
	if not name:
		frappe.throw(_("Смена {0} не найдена").format(external_id))
	return name
...
def _set_doc_scope(doc, connection, shift):
	shift_scope = frappe.db.get_value(
		"Sales Shift", shift, ["business_entity", "business_point", "warehouse", "cashier"], as_dict=True
	)
	doc.business_entity, doc.business_point, doc.warehouse = (
		shift_scope.business_entity,
		shift_scope.business_point,
		shift_scope.warehouse,
	)
...
def _receipt_rows(filters, or_filters=None, limit=10000):
	...
	payments = frappe.get_all(
		"Sales Receipt Payment",
		filters={"parent": ["in", parents or ["__none__"]]},
		fields=["parent", "payment_channel", "amount"],
		limit_page_length=100000,
	)
	by_parent = {}
	for p in payments:
		by_parent.setdefault(p.parent, {})[p.payment_channel] = by_parent.setdefault(p.parent, {}).get(
			p.payment_channel, 0
		) + flt(p.amount)
	for row in rows:
		row["payments"] = by_parent.get(row.name, {})
	return rows
...
def _scope_filters():
	...
	return {"active": 1, "name": ["in", get_allowed_entities(scope) or ["__none__"]]}, {
		"active": 1,
...
def _business_point_filters(business_entity=None, business_point=None):
	...
	if not scope["global"]:
		...
		if business_entity and business_entity not in allowed_entities:
...
def _ensure_point(point, entity=None):
	...
	if entity and frappe.db.get_value("Business Point", point, "business_entity") != entity:
		frappe.throw(_("Точка не относится к выбранному ИП"))


def _live_connection_status(connection):
	if not cint(connection.enabled):
		return "Не подключена"
	if connection.status == "Ошибка":
		return "Ошибка"
	if not connection.last_seen_at:
		return "Не подключена"
	return (
		"Нет связи"
		if now_datetime() - get_datetime(connection.last_seen_at) > timedelta(minutes=15)
		else "В сети"
	)
