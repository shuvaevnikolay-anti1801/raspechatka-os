...
def push_events(workplace_code=None, events=None):
    ...
    for event in events:
        ...
        if not frappe.db.exists("POS Event", event_id):
            frappe.get_doc(
                {
                    ...
                    "occurred_at": get_datetime(event.get("createdAt"))
            ...
            _apply_pos_event(
                event_type, event_id, workplace, event.get("payload") or {}
            )
...
def _get_employee():
    ...
    frappe.throw(
        "Для пользователя не создан активный сотрудник", frappe.PermissionError
    )
...
def _get_products(point_name):
    ...
    warehouse = frappe.db.get_value(
        "Catalog Warehouse", {"business_point": point_name, "active": 1}, "name"
    )
    balances = (
        dict(
            frappe.db.sql(
    ...
    for assortment in assortments:
        ...
        if not resolved_price and not frappe.db.get_value(
            "Business Point", point_name, "allow_free_price"
        ):
            continue
    ...
    return [
        {
            ...
            "purchaseCount": int(
                (purchases.get(row.name) or {}).get("purchase_count") or 0
            ),
            "totalSpentMinor": round(
                flt((purchases.get(row.name) or {}).get("total_spent")) * 100
            ),
        }
        for row in frappe.get_all(
            "Client",
...
def _apply_order_created(event_id, workplace, payload):
    if not _doctype_exists("POS Order") or frappe.db.exists(
        "POS Order", {"source_pos_event": event_id}
    ):
        return
    status = {
        "new": "New",
            ...
            "items": [
                {
                    ...
                    "amount": flt(x.get("quantity"))
                    * flt(x.get("unitPriceMinor"))
                    / 100,
                }
...
def _apply_order_updated(event_id, workplace, payload):
    ...
    name = frappe.db.get_value(
        "POS Order", {"order_number": payload.get("orderNumber")}, "name"
    )
    if not name:
        return
...
def _apply_sale(event_id, workplace, payload):
    ...
    gross = sum(
        flt(line.get("quantity")) * flt(line.get("unitPriceMinor")) / 100
        for line in lines
    )
    if (
        client
        and frappe.db.exists("Client", client)
        and not frappe.db.exists("Client Purchase", {"source_document": sale_id})
    ):
        frappe.get_doc(
            {
                "doctype": "Client Purchase",
                "purchase_datetime": get_datetime(payload.get("createdAt"))
                ...
                "items": [
                    {
                        "item": line.get("productId")
                        if frappe.db.exists("Catalog Item", line.get("productId"))
                        else None,
                        "item_name": line.get("name"),
                        "quantity": flt(line.get("quantity")),
                        "rate": flt(line.get("unitPriceMinor")) / 100,
                        "amount": flt(line.get("quantity"))
                        * flt(line.get("unitPriceMinor"))
                        / 100,
                    }
                    for line in lines
                ],
...
def _apply_return(event_id, workplace, payload):
    ...
    purchase_name = frappe.db.get_value(
        "Client Purchase", {"source_document": sale_id}, "name"
    )
    if purchase_name:
        purchase = frappe.get_doc("Client Purchase", purchase_name)
        purchase.returned_amount = min(
            flt(purchase.net_amount), flt(purchase.returned_amount) + total
        )
        purchase.status = (
            "Returned"
            if purchase.returned_amount >= flt(purchase.net_amount)
    ...
    _create_stock_entries(
        workplace, return_id, "POS Return", payload.get("lines") or [], multiplier=1
    )
...
def _get_supply_requests(point_name):
    ...
    return [
        ...
        for row in frappe.get_all(
            ...
            filters={
                "business_point": point_name,
                "status": ["not in", ["Закрыта", "Отклонена"]],
            },
...
def _point_context(workplace):
    point = frappe.get_doc("Business Point", workplace.business_point)
    warehouse = frappe.db.get_value(
        "Catalog Warehouse", {"business_point": point.name, "active": 1}, "name"
    )
    ...
    frappe.get_doc(
        {
            ...
            and frappe.db.exists("Catalog Item", payload.get("productId"))
            else None,
...
def _apply_cleaner_payment(event_id, workplace, payload):
    ...
    if len(visits) < 4:
        frappe.throw(
            "Для выплаты должно быть зарегистрировано четыре неоплаченных посещения"
        )
    for visit in visits:
        frappe.db.set_value(
            "Cleaner Visit",
            visit,
            {"paid": 1, "paid_at": now(), "payment_pos_event": event_id},
        )
    ...
    frappe.get_doc(
        {
            ...
            "business_point": workplace.business_point,
            "pos_workplace": workplace.name,
            "counted_at": get_datetime(payload.get("createdAt"))
            if payload.get("createdAt")
            else now(),
            ...
            "lines": [
                {
                    "denomination": flt(row.get("denominationMinor")) / 100,
