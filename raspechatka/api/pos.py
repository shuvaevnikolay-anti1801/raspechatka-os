...
def _get_orders(point_name):
	if not _doctype_exists("POS Order"):
		return []
	rows = frappe.get_all("POS Order", filters={"business_point": point_name, "creation": [">=", add_days(now_datetime(), -60)]}, fields=["name", "order_number", "phone", "customer_name", "total_amount", "paid_amount", "status", "comment", "due_at", "creation", "source_sale_id", "fiscal_number"], order_by="creation desc", limit_page_length=5000)
	items = {}
	for item in frappe.get_all("POS Order Item", filters={"parent": ["in", [row.name for row in rows] or ["__none__"]]}, fields=["parent", "item", "item_name", "quantity", "rate"], limit_page_length=5000):
		items.setdefault(item.parent, []).append({"productId": item.item, "name": item.item_name, "quantity": flt(item.quantity), "unitPriceMinor": round(flt(item.rate) * 100)})
	status = {"New": "new", "In Progress": "in_progress", "Ready": "ready", "Issued": "issued", "Cancelled": "cancelled"}
	return [{"id": x.name, "orderNumber": x.order_number, "phone": x.phone, "customerName": x.customer_name, "lines": items.get(x.name, []), "totalMinor": int(flt(x.total_amount) * 100), "paidMinor": int(flt(x.paid_amount) * 100), "paymentStatus": "paid" if flt(x.paid_amount) >= flt(x.total_amount) else ("partial" if flt(x.paid_amount) else "unpaid"), "status": status.get(x.status, "new"), "comment": x.comment, "createdAt": str(x.creation), "dueAt": x.due_at, "sourceSaleId": x.source_sale_id, "fiscalNumber": x.fiscal_number} for x in rows]
