...
def _ensure_receipt(name, docstatus):
	if not frappe.db.exists("Stock Receipt", {"name": name, "docstatus": docstatus, **_receipt_scope_filters()}):
...
def _options():
	scope = get_scope()
	point_filter = {"active": 1} if scope["global"] else {"active": 1, "name": ["in", scope["points"] or ["__none__"]]}
	points = frappe.get_all("Business Point", filters=point_filter, fields=["name", "point_name", "business_entity"], order_by="point_name asc")
	...
	if scope["global"]:
		...
		entities = frappe.get_all("Business Entity", filters={"active": 1, "name": ["in", entity_names or ["__none__"]]}, fields=["name", "short_name"], order_by="short_name asc")
	warehouses = frappe.get_all("Catalog Warehouse", filters={"active": 1, "business_point": ["in", point_names]}, fields=["name", "warehouse_name", "business_point"], order_by="warehouse_name asc")
	supplier_filters = _supplier_scope_filters()
	items = frappe.get_all("Catalog Item", filters={"active": 1, "item_type": "Product", "track_inventory": 1}, fields=["name", "item_code", "item_name", "stock_uom"], order_by="item_name asc", limit_page_length=3000)
	locations = frappe.get_all("Storage Location", filters={"active": 1, "warehouse": ["in", [row.name for row in warehouses] or ["__none__"]]}, fields=["name", "location_name", "full_address", "warehouse"], order_by="full_address asc", limit_page_length=3000)
	defaults = frappe.get_all("Catalog Item Storage", filters={"active": 1, "warehouse": ["in", [row.name for row in warehouses] or ["__none__"]]}, fields=["item", "warehouse", "storage_location"], limit_page_length=5000)
	return {
		"entities": entities,
		"points": points,
		"warehouses": warehouses,
		"suppliers": frappe.get_all("Catalog Supplier", filters=supplier_filters, fields=["name", "supplier_name"], order_by="supplier_name asc"),
		"items": items,
		"locations": locations,
