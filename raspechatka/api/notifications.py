		...
		items.append(
			{
				...
				"published_label": frappe.format_value(row.published_from or row.creation, {"fieldtype": "Datetime"}),
...
def save_announcement(data):
	...
	doc = frappe.get_doc("System Announcement", data["name"]) if data.get("name") else frappe.new_doc("System Announcement")
	for fieldname in (
		"title",
		"message",
		"priority",
