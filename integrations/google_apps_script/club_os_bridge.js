...
const CLUB_OS = Object.freeze({
  ...
  batch: 15,
  headers: ['Event ID', 'Client ID', 'Hash', 'Payload', 'Status', 'Attempts', 'Error', 'Sent at'],
});
...
function clubOSObjects_(sheet) {
  ...
  return values.filter(row => row.some(v => v !== '' && v !== null)).map(row => {
    ...
    names.forEach((key, index) => {
      if (!key) return;
      const v = row[index];
      result[key] = v instanceof Date
        ? Utilities.formatDate(v, SpreadsheetApp.getActiveSpreadsheet().getSpreadsheetTimeZone(), 'yyyy-MM-dd HH:mm:ss')
        : v;
    });
    return result;
  });
...
function clubOSHash_(text) {
  return clubOSHex_(Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, text, Utilities.Charset.UTF_8));
}
...
function clubOSSnapshot_() {
  ...
  const grouped = {};
  [['channels', 'Каналы'], ['consents', 'Согласия'], ['events', 'Лог событий']].forEach(([kind, title]) => {
    grouped[kind] = {};
...
function clubOSSend_(payload) {
  const secret = PropertiesService.getScriptProperties().getProperty('RASPECHATKA_OS_SECRET') || '';
  if (secret.length < 32) throw new Error('Задайте RASPECHATKA_OS_SECRET в свойствах скрипта');
  const timestamp = String(Math.floor(Date.now() / 1000));
  const signature = clubOSHex_(Utilities.computeHmacSha256Signature(timestamp + '\n' + payload, secret, Utilities.Charset.UTF_8));
  const response = UrlFetchApp.fetch(CLUB_OS.url, {
    method: 'post', contentType: 'application/json',
    payload: JSON.stringify({payload, timestamp, signature}), muteHttpExceptions: true,
  });
  if (response.getResponseCode() !== 200) throw new Error('OS HTTP ' + response.getResponseCode());
...
function clubOSInstall() {
  ...
  if ((props.getProperty('RASPECHATKA_OS_SECRET') || '').length < 32) throw new Error('Сначала задайте ключ');
  clubOSQueue_();
...
function clubOSTick() {
  ...
  try {
    ...
    const pendingIds = new Set(pending.filter(r => r.Status !== 'SENT').map(r => String(r['Client ID'])));
    let added = 0;
    ...
    for (const snapshot of snapshots) {
      ...
      if (payload.length > 45000) throw new Error('Снимок клиента превышает лимит ячейки; требуется пакетный импорт истории');
    ...
    for (let i = 1; i < rows.length && sent < CLUB_OS.batch; i++) {
      ...
      if (row[4] === 'ERROR' && row[7] instanceof Date && Date.now() - row[7].getTime() < Math.min(3600000, 60000 * Math.pow(2, Math.min(Number(row[5]), 6)))) continue;
      try {
        const result = clubOSSend_(String(row[3]));
        props.setProperty('CLUB_OS_HASH_' + row[1], String(row[2]));
        sheet.getRange(i + 1, 5, 1, 4).setValues([['SENT', Number(row[5]) + 1, result.outcome === 'DIFFERENCE' ? 'Расхождение скидки/каналов' : '', new Date()]]);
        sheet.getRange(i + 1, 4).clearContent(); // Remove tokens after acknowledgement.
      } catch (error) {
        sheet.getRange(i + 1, 5, 1, 4).setValues([['ERROR', Number(row[5]) + 1, String(error.message).slice(0, 200), new Date()]]);
      }
      sent++;
    }
  } finally {
