...
function clubOSSnapshot_() {
  ...
  const clients = clubOSObjects_(ss.getSheetByName("Клиенты")).filter(
    (c) => c["Client ID"]
  );
  const grouped = {};
  [
...
function clubOSSend_(payload) {
  const secret =
    PropertiesService.getScriptProperties().getProperty(
      "RASPECHATKA_OS_SECRET"
    ) || "";
  if (secret.length < 32)
    throw new Error("Задайте RASPECHATKA_OS_SECRET в свойствах скрипта");
...
function clubOSInstall() {
  ...
  if (
    !ScriptApp.getProjectTriggers().some(
      (t) => t.getHandlerFunction() === "clubOSTick"
    )
  ) {
