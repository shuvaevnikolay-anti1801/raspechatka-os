...
const visibleLocations = (row) =>
	options.locations.filter((location) => location.warehouse === form.warehouse);
...
const title = computed(
	() => `${form.receipt_type || "Приёмка"}${form.name ? ` № ${form.name}` : ""}`
);
const filterFields = computed(() => [
	{
		key: "search",
		label: "Поиск",
		placeholder: "Номер, поставщик или входящий документ",
		wide: true,
	},
	{
		key: "receipt_type",
		...
		type: "select",
		allLabel: "Все типы",
		options: ["Приёмка", "Оприходование"].map((value) => ({ value, label: value })),
	},
	{
		key: "status",
		label: "Статус",
...
const listColumns = [
	...
	{ key: "receipt_type", label: "Тип", width: 140 },
	{ key: "posting_datetime", label: "Дата", format: dateTime, width: 150 },
	{ key: "business_point", label: "Точка", format: pointLabel, width: 180 },
...
async function load(page = 1, size = pageSize.value) {
	...
	error.value = "";
	try {
		const result = await call("raspechatka.api.warehouse.get_receipts", {
			...filters.value,
...
async function loadOptions() {
	try {
		const result = await call("raspechatka.api.warehouse.get_receipt", {
...
function addLine() {
	form.items.push({ item: "", uom: "", storage_location: "", quantity: 1, rate: "" });
}
...
function onItemChange(row) {
	...
	const preferred = options.storage_defaults.find(
...
function refreshAddresses() {
	for (const row of form.items || []) onItemChange(row);
}
...
async function submit() {
	...
	try {
		...
		await call(
			"raspechatka.api.warehouse.submit_receipt",
			{ name: form.name },
			{ method: "POST" }
		);
		await Promise.all([load(), openReceipt(form.name)]);
	} catch (e) {
		formError.value = e.message;
	} finally {
		saving.value = false;
...
async function cancel() {
	...
	try {
		await call(
			"raspechatka.api.warehouse.cancel_receipt",
			{ name: form.name },
			{ method: "POST" }
		);
		await Promise.all([load(), openReceipt(form.name)]);
...
<template>
	<section class="page warehouse-page">
		<ListPageHeader title="Приёмки и оприходования"
			><template #actions>
				<div v-if="canEdit" class="heading-actions">
					<button
						class="button button-secondary"
						@click="openReceipt(null, 'Оприходование')"
					>
						＋ Оприходование
					</button>
					<button class="button button-primary" @click="openReceipt(null, 'Приёмка')">
						＋ Приёмка
					</button>
				</div>
		...
		<SmartDataTable
			...
			><template #cell-receipt_type="{ row }"
				><span
					class="type-chip"
					:class="{ service: row.receipt_type === 'Оприходование' }"
					>{{ row.receipt_type }}</span
				></template
		...
		<AppModal v-if="editorOpen" :title="title" wide @close="editorOpen = false">
			<form class="receipt-form" @submit.prevent="save(false)">
				<div class="document-strip">
					...
					><span v-else>Новый документ</span
					><span v-if="form.purchase_order">По заказу {{ form.purchase_order }}</span>
				</div>
				<div class="form-section">
					<h3>Основное</h3>
					<div class="form-grid">
						<label
							>Тип документа<select
								v-model="form.receipt_type"
								:disabled="form.docstatus !== 0 || !!form.purchase_order"
							>
								<option>Приёмка</option>
								<option>Оприходование</option>
							</select></label
						>
						<label
							>Дата и время<input
						...
						<template v-if="form.receipt_type === 'Приёмка'">
							...
							>
							<label
								>Номер документа поставщика<input
									v-model="form.supplier_document_number"
									:disabled="form.docstatus !== 0"
							/></label>
							<label
								>Дата документа поставщика<input
									v-model="form.supplier_document_date"
									type="date"
									:disabled="form.docstatus !== 0"
							/></label>
						</template>
						<label v-else class="span-2"
							>Основание оприходования<input
								v-model="form.reason"
								:disabled="form.docstatus !== 0"
								required
								placeholder="Например: обнаруженные излишки"
						/></label>
					</div>
				</div>
				<div class="form-section receipt-items">
					<div class="section-heading">
						<div>
							<h3>Товары</h3>
							<small>Закупочная цена обязательна и формирует себестоимость</small>
						</div>
						<button
					...
					<div v-if="!(form.items || []).length" class="line-empty">
						Добавьте товары из каталога
					</div>
					<div v-else class="receipt-lines">
						<div class="receipt-line receipt-line-head">
							<span>Товар</span><span>Ед.</span><span>Место хранения</span
							><span>Кол-во</span><span>Цена</span><span>Сумма</span><span></span>
						</div>
						<div
						...
						>
							<select
								v-model="row.item"
								:disabled="form.docstatus !== 0 || !!row.purchase_order_item"
								required
							...
							>
								...
								<option
									v-for="location in visibleLocations(row)"
									:key="location.name"
									:value="location.name"
								>
