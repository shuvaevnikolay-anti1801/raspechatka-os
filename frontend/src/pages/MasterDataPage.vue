...
const form = reactive({}),
	options = reactive({ entities: [], points: [], positions: [], products: [] });
const supplierBank = reactive({
	bank_name: "",
	bic: "",
	settlement_account: "",
	currency: "RUB",
...
const configs = {
	organizations: {
		title: "Партнёры",
		description: "Партнёры сети, объединяющие одно или несколько юридических лиц",
		create: "Добавить партнёра",
		entityFields: defineEntityFields([
			{
				key: "search",
				label: "Поиск",
	...
	clients: {
		title: "Клиенты",
		description: "Единая клиентская база всей сети",
		...
		fields: [
			f("last_name", "Фамилия"),
			f("first_name", "Имя", "text", 1),
			f("middle_name", "Отчество"),
			f("phone", "Телефон", "text", 1),
			f("email", "Email", "email"),
			f("registration_point", "Точка регистрации", "link", 1, "points", "point_name"),
			f("personal_data_consent", "Согласие на обработку данных", "check"),
			f("marketing_consent", "Согласие на рассылку", "check"),
			f("notes", "Комментарий", "textarea"),
		],
	},
	suppliers: {
		title: "Поставщики",
		description: "Общесетевые и собственные поставщики ИП",
		...
		fields: [
			...
			f("supplier_type", "Тип", "select", 1, [
				{ value: "Company", label: "Компания" },
				{ value: "Individual Entrepreneur", label: "ИП" },
				{ value: "Individual", label: "Физлицо" },
			]),
			f("scope", "Доступность", "select", 1, [
				{ value: "Network", label: "Вся сеть" },
				{ value: "Business Entity", label: "Собственный у ИП" },
			]),
			f(
				"business_entity",
				"Владелец-ИП",
			...
			f("email", "Email", "email"),
			f("website", "Сайт"),
			f("contact_name", "Основное контактное лицо"),
			f("contact_position", "Должность"),
			f("contact_phone", "Телефон контактного лица"),
			f("contact_email", "Email контактного лица", "email"),
			f("notes", "Комментарий", "textarea"),
		],
	},
	employees: {
		title: "Сотрудники",
		description: "Кадровый учёт трудоустроенных сотрудников",
		...
		fields: [
			...
			f("gender", "Пол", "select", 0, [
				{ value: "Мужской", label: "Мужской" },
				{ value: "Женский", label: "Женский" },
			]),
			f("business_entity", "Работодатель-ИП", "link", 1, "entities", "short_name"),
			f("position", "Должность", "link", 1, "positions", "position_name"),
			f("employment_type", "Тип оформления", "select", 1, [
				{ value: "Трудовой договор", label: "Трудовой договор" },
				{ value: "ГПХ", label: "ГПХ" },
				{ value: "Самозанятый", label: "Самозанятый" },
				{ value: "ИП", label: "ИП" },
				{ value: "Без оформления", label: "Без оформления" },
			]),
			f("hire_date", "Дата приёма", "date"),
			f("dismissal_date", "Дата увольнения", "date"),
			f("notes", "Комментарий", "textarea"),
		],
	},
	positions: {
		title: "Должности",
		description: "Единый справочник должностей сети",
	...
	"catalog-groups": {
		...
		description: "Иерархия товаров и услуг",
		...
		fields: [
			...
			f("parent_catalog_group", "Родительская группа"),
			f("is_group", "Может содержать группы", "check"),
		],
	},
	"catalog-units": {
		title: "Единицы измерения",
		description: "Общие единицы измерения сети",
		create: "Добавить единицу",
		columns: cols(
			["unit_name", "Единица", 1],
			["symbol", "Обозначение"],
		...
		fields: [
			...
			f("symbol", "Обозначение", "text", 1),
			f("allow_fraction", "Разрешить дробное количество", "check"),
		],
	},
	"price-types": {
		title: "Типы цен",
		description: "Системные цены продажи и закупки",
		create: "Добавить тип",
		columns: cols(
		...
		fields: [
			...
			f("purpose", "Назначение", "select", 1, [
				{ value: "Selling", label: "Продажа" },
				{ value: "Buying", label: "Закупка" },
			]),
			f("currency", "Валюта", "text", 1),
		],
	},
	"financial-articles": {
		title: "Финансовые статьи",
		description: "Единые статьи доходов и расходов",
		...
		fields: [
			...
			f("article_type", "Тип", "select", 1, [
				{ value: "Income", label: "Доход" },
				{ value: "Expense", label: "Расход" },
			]),
			f("parent_financial_article", "Родительская статья"),
			f("is_group", "Это группа", "check"),
			f("description", "Описание", "textarea"),
		],
	},
	"payment-methods": {
...
const areas = {
	organizations: "references.network",
	clients: "references.clients",
	suppliers: "references.suppliers",
	employees: "references.employees",
	positions: "references.employees",
	"catalog-groups": "references.catalog",
	"catalog-units": "references.catalog",
	"price-types": "references.catalog",
	"financial-articles": "references.finance",
	"payment-methods": "references.finance",
...
function reset(values = {}) {
	...
	Object.assign(form, { active: 1, ...values });
}
...
function defaults() {
	if (reference.value === "clients")
		return { personal_data_consent: 0, marketing_consent: 0, messengers: [] };
	if (reference.value === "suppliers") return { supplier_type: "Company", scope: "Network" };
	if (reference.value === "employees") return { employment_type: "Трудовой договор" };
	if (reference.value === "price-types") return { purpose: "Selling", currency: "RUB" };
	return {};
}
async function open(row) {
	if (!config.value.create) return;
	try {
		const data = await call("raspechatka.api.references.get_reference_detail", {
			reference: reference.value,
			name: row.name,
		});
		detail.value = data;
		...
		if (reference.value === "clients") {
			for (const m of form.messengers || [])
				form[`messenger_${m.messenger_type}`] = m.contact;
		}
	} catch (e) {
		error.value = e.message;
	}
...
async function save() {
	...
	try {
		if (reference.value === "clients")
			form.messengers = ["Telegram", "WhatsApp", "MAX"]
				.filter((t) => form[`messenger_${t}`])
				.map((t) => ({ messenger_type: t, contact: form[`messenger_${t}`] }));
		const r = await call(
...
<template>
	<section class="page reference-page">
		...
		<ReferenceTable
			...
			@retry="load"
		/>
		<AppModal v-if="detail !== null" :title="config.title" wide @close="detail = null"
			><form class="editor-form" @submit.prevent="save">
				<div class="form-section">
					...
					<div class="form-grid">
						<label
							...
							:key="field.key"
							:class="{ 'span-3': field.type === 'textarea' }"
							>{{ field.label
							}}<textarea
							...
							></textarea
							><select
								v-else-if="field.type === 'select' || field.type === 'link'"
								v-model="form[field.key]"
								:required="field.required"
							>
								<option value="">Не выбрано</option>
								<option
									v-for="o in fieldOptions(field)"
									:key="o.value"
									:value="o.value"
								>
									{{ o.label }}
				...
				<div v-if="reference === 'clients'" class="form-section">
					<h3>Мессенджеры</h3>
					<div class="form-grid">
						<label v-for="m in ['Telegram', 'WhatsApp', 'MAX']" :key="m"
							>{{ m }}<input v-model="form[`messenger_${m}`]"
						/></label>
					</div>
				</div>
				<template v-if="reference === 'suppliers' && form.name"
					><div class="form-section">
						<h3>Банковские счета</h3>
						<div class="compact-list">
							<div v-for="a in detail.bank_accounts" :key="a.name">
								<b>{{ a.bank_name }}</b
								><span>{{ a.settlement_account }}</span>
							</div>
						</div>
						<div v-if="canEdit" class="inline-form bank-inline">
							<input v-model="supplierBank.bank_name" placeholder="Банк" /><input
								v-model="supplierBank.bic"
								placeholder="БИК"
							/><input
								v-model="supplierBank.settlement_account"
					...
					<div class="form-section">
						...
						<div class="compact-list">
							<div v-for="i in detail.items" :key="i.name">
								<b>{{ i.item }}</b
								><span>{{ i.is_primary ? "Основной" : "Дополнительный" }}</span>
							</div>
						</div>
						<div v-if="canEdit" class="inline-form">
							<select v-model="supplierItem.item">
							...
							><label class="check-field"
								><input
									v-model="supplierItem.is_primary"
									type="checkbox"
									:true-value="1"
									:false-value="0"
								/>Основной</label
