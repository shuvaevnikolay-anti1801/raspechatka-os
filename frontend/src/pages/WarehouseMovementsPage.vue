...
const filterFields = computed(() => [
	...
	{ key: "business_point", label: "Точка", type: "select", allLabel: "Все точки", options: options.points.map((row) => ({ value: row.name, label: row.point_name })) },
...
const quantity = (value) => new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 3 }).format(Number(value || 0));
const money = (value) => `${new Intl.NumberFormat("ru-RU", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(value || 0))} ₽`;
const date = (value) => value ? new Intl.DateTimeFormat("ru-RU", { dateStyle: "short", timeStyle: "short" }).format(new Date(String(value).replace(" ", "T"))) : "—";
const columns = [
	...
	{ key: "stock_value_difference", label: "Стоимость движения", number: true, format: money },
	{ key: "stock_value_after", label: "Стоимость остатка", number: true, format: money },
	{ key: "valuation_source", label: "Источник расчёта" },
	{ key: "voucher_no", label: "Исходный документ" },
];

function sourceUrl(row) {
	const slug = String(row.voucher_type || "").trim().toLowerCase().replaceAll(" ", "-");
	...
	<section class="page report-page">
		...
		>
			<template #cell-voucher_no="{ row }">
				<a v-if="sourceUrl(row)" :href="sourceUrl(row)" target="_blank" rel="noopener" @click.stop>{{ row.voucher_no }}</a>
