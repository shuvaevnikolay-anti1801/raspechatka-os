...
const visibleWarehouses = computed(() =>
  options.warehouses.filter(
    (row) =>
      !filters.business_point || row.business_point === filters.business_point,
  ),
);
...
function sourceUrl(row) {
  const slug = String(row.voucher_type || "")
    ...
    .toLowerCase()
    .replaceAll(" ", "-");
  return slug && row.voucher_no
    ? `/app/${slug}/${encodeURIComponent(row.voucher_no)}`
    : "";
}
async function load(page = 1, size = pageSize.value) {
  ...
  try {
    const result = await call(
      "raspechatka.api.warehouse_reports.get_stock_movements",
      {
        ...filters,
...
async function init() {
  try {
    Object.assign(
      options,
      await call("raspechatka.api.warehouse_reports.get_report_options"),
    );
    await load();
  } catch (exception) {
    error.value = exception.message;
