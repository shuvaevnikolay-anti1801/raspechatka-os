...
const filterFields = computed(() => [
	...
	},
	{
		...
		type: "select",
		allLabel: "Все точки",
		options: pointsFor(filters.value.business_entity).map((item) => ({
			value: item.name,
...
const columns = [
	...
	{ key: "business_point", label: "Точка", width: 180, displayKey: "business_point_label" },
	{
		...
		label: "Тип",
		width: 120,
		format: (value) => (value === "Income" ? "Поступление" : "Оплата"),
	},
...
async function load() {
	...
	error.value = "";
	try {
		const result = await call("raspechatka.api.finance.get_payment_calendar", {
...
function edit(row = null) {
	...
	Object.assign(
		...
		row
			...
			: {
					title: "",
					planned_date: `${filters.value.month}-01`,
					direction: "Expense",
					amount: 0,
					currency: "RUB",
					status: "Planned",
...
<template>
	<section class="page finance-page">
		<ListPageHeader title="Платёжный календарь"
			><template #actions
				><button v-if="canEdit" class="button button-primary" @click="edit()">
					＋ Запланировать
				</button></template
			></ListPageHeader
		>
		<div class="finance-summary">
			<article>
				<span>ВСЕГО К ОПЛАТЕ</span><b>{{ money(totals.planned) }}</b>
			</article>
			<article class="accent">
				<span>ОПЛАЧЕНО</span><b>{{ money(totals.paid) }}</b>
			</article>
			<article>
				<span>ОСТАЛОСЬ</span><b>{{ money(totals.remaining) }}</b>
			</article>
		</div>
		<SmartFilterBar
			v-model="filters"
		...
		<SmartDataTable
			...
			:loading="loading"
			:error="error"
			empty-title="На этот месяц ничего не запланировано"
			empty-text="Добавьте регулярные и разовые платежи."
			@open="edit"
			@retry="load"
		>
			<template #cell-display_status="{ row }"
				><span
					...
					:class="`plan-${row.display_status.toLowerCase()}`"
					>{{
		...
		<AppModal
			v-if="open"
			:title="form.name ? 'Плановый платёж' : 'Новый плановый платёж'"
			@close="open = false"
			><form class="editor-form" @submit.prevent="save">
				<div class="form-section">
					<div class="form-grid">
						<label class="span-2"
							>Название<input v-model="form.title" required /></label
						><label
							>Дата<input v-model="form.planned_date" type="date" required /></label
						><label
							>Тип<select v-model="form.direction">
						...
						><label
							>ИП<select v-model="form.business_entity" required>
								<option
						...
						><label
							>Точка<select v-model="form.business_point">
						...
						><label
							>Статья<select v-model="form.financial_article" required>
								<option
									v-for="item in articlesFor(form.direction)"
									:key="item.name"
									:value="item.name"
								>
									{{ item.article_name }}
						...
						><label>Контрагент<input v-model="form.counterparty_name" /></label
						><label
							>Повторение<select v-model="form.recurrence">
								<option value="Once">Один раз</option>
								<option value="Monthly">Каждый месяц</option>
							</select></label
						><label class="span-3"
							>Комментарий<textarea v-model="form.comment"></textarea>
						</label>
					</div>
				</div>
			</form>
			<template #footer
				><div>
					<button
						v-if="form.name && canEdit && form.status !== 'Paid'"
				...
				<div class="footer-actions">
					<button class="button button-secondary" @click="open = false">Закрыть</button
					><button
						v-if="canEdit"
