	...
	Object.fromEntries(
		days.value.map((day) => {
			...
			for (const employee of selectedPointEmployees.value) {
				...
				if (
					value === BOTH ||
					(morningShift.value?.name && value === morningShift.value.name)
				)
					morning += 1;
				if (
					value === BOTH ||
					(eveningShift.value?.name && value === eveningShift.value.name)
				)
					evening += 1;
			}
...
const plannedHours = computed(() =>
	Object.fromEntries(
		selectedPointEmployees.value.map((employee) => [
			employee.name,
...
const filterModel = computed({
	...
	set: (value) => {
		point.value = value.business_point;
...
const filterFields = computed(() => [
	{
		...
		label: "Точка",
		type: "select",
		allLabel: "Все доступные точки",
		options: data.value.points.map((item) => ({
			value: item.name,
			label: `${item.point_name} · ${item.city || ""}`,
		})),
	},
...
const employeeColumns = computed(() => [
	...
	{ key: "business_entity", label: "Работодатель", width: 200 },
	{
		...
		width: 220,
		format: (value) => pointNames.value[value] || "Не назначена",
	},
...
const leaveColumns = computed(() => [
	{
		...
		primary: true,
		width: 230,
		format: (v) => hr.value.employee_names[v] || v,
	},
...
function cellKey(employee, day) {
	return `${employee}|${isoDate(day)}`;
}
function isPastDay(day) {
	return Boolean(schedule.value.today && isoDate(day) < schedule.value.today);
}
...
function isWeekend(day) {
	return [0, 6].includes(dayOfWeek(day));
}
function isWeekStart(day) {
	return day > 1 && dayOfWeek(day) === 1;
}
function shiftLabel(shift) {
	return { U: "У", V: "В" }[shift?.shift_code] || shift?.shift_code || "—";
}
...
async function saveSchedule() {
	...
	for (const [key, value] of Object.entries(draft.value)) {
		...
		const shifts =
			value === BOTH
				? baseShifts.value
				: data.value.shift_templates.filter((shift) => shift.name === value);
		for (const shift of shifts) entries.push({ employee, date, shift_template: shift.name });
	}
	try {
		await call(
			"raspechatka.api.team.save_schedule",
			{
				business_point: point.value,
				month: `${month.value}-01`,
...
async function calculatePayroll(save = false) {
	if (!point.value) {
		error.value = "Выберите точку";
		return;
	}
	saving.value = true;
	error.value = "";
	try {
		payroll.value = await call(
			"raspechatka.api.team.calculate_payroll",
			{
				business_point: point.value,
	...
	<section class="page team-page">
		<ListPageHeader :title="title">
			<template #actions>
				...
				>
				<template v-if="section === 'payroll'"
					><button class="button" :disabled="saving" @click="calculatePayroll(false)">
						Рассчитать</button
					><button
						class="button button-primary"
						:disabled="saving || !payroll"
						@click="calculatePayroll(true)"
					>
						Сохранить ведомость
					</button></template
				>
				<a v-if="section === 'hr'" class="button button-primary" href="/app/employee-leave"
					>Оформить отпуск</a
				>
			</template>
		</ListPageHeader>
		...
		<template v-else>
			<div class="team-stats">
				<article>
					...
					><b>{{ data.counters.active_employees || 0 }}</b>
				</article>
				<article>
					<span>Точки в доступе</span><b>{{ data.counters.points || 0 }}</b>
				</article>
				<article>
					<span>Опубликовано графиков</span
					><b>{{ data.counters.published_schedules || 0 }}</b>
				</article>
				<article>
					<span>Активные игры</span><b>{{ data.counters.active_games || 0 }}</b>
				</article>
			</div>
			...
			<div v-if="section === 'employees'" class="team-panel">
				<SmartDataTable
					...
					empty-text="Создайте первую кадровую карточку."
				/>
			</div>
			...
			<div v-else-if="section === 'schedule'" class="team-panel schedule-panel">
				<div class="panel-title">
					<div>
						<h2>План и факт смен</h2>
						<p>
							На каждый день назначьте ровно одну утреннюю и одну вечернюю смену.
							«У/В» назначает обе смены одному сотруднику.
						</p>
					</div>
					<span class="status">{{ schedule.status || "Новый" }}</span>
				</div>
				<div v-if="!point" class="team-empty"><b>Выберите точку</b></div>
				<template v-else>
					<div class="schedule-edit-controls">
						<label
							><input v-model="allowPastEditing" type="checkbox" /> Разрешить
							изменение прошедших дней</label
						>
						<span>После сохранения защита включится автоматически.</span>
					</div>
					<div
						class="coverage-summary"
						:class="{ warning: completeDays !== days.length }"
					>
						<b>Заполнено {{ completeDays }} из {{ days.length }} дней</b
						><span v-if="completeDays !== days.length"
							>Красным отмечены дни без полного покрытия. Это не мешает
							сохранению.</span
						><span v-else>График заполнен полностью.</span>
					</div>
					<div class="schedule-scroll">
						<table class="schedule-grid">
							<thead>
								<tr class="weekday-row">
									<th rowspan="2" class="employee-col">Сотрудник</th>
									<th
										v-for="day in days"
										:key="day"
										:class="{
											weekend: isWeekend(day),
											'week-start': isWeekStart(day),
										}"
									>
										{{ weekdayLabel(day) }}
									</th>
								...
								<tr class="day-row">
									<th
										...
										:key="day"
										:class="{
											'coverage-bad': !dayCoverage[day]?.ok,
							...
							<tbody>
								<tr
									v-for="employee in selectedPointEmployees"
									:key="employee.name"
								>
									<th class="employee-col">{{ employee.employee_name }}</th>
									<td
										v-for="day in days"
										:key="day"
										:class="[
											{
												'coverage-bad-soft': !dayCoverage[day]?.ok,
												'past-day': isPastDay(day),
												weekend: isWeekend(day),
												'week-start': isWeekStart(day),
											},
											shiftCellClass(employee.name, day),
										]"
									>
										<select
											v-model="draft[cellKey(employee.name, day)]"
											:disabled="isPastDay(day) && !allowPastEditing"
											:title="
												isPastDay(day) && !allowPastEditing
													? 'Прошедшие смены изменять нельзя'
													: isoDate(day)
											"
										>
											<option value="">—</option>
											<option
												v-for="shift in baseShifts"
												:key="shift.name"
												:value="shift.name"
											>
												{{ shiftLabel(shift) }}
											</option>
											<option v-if="baseShifts.length === 2" :value="BOTH">
												У/В
											</option>
										</select>
									</td>
							...
							<tfoot>
								<tr>
									<th class="employee-col">Проверка дня</th>
									<td
										v-for="day in days"
										:key="day"
										:class="{
											'coverage-ok': dayCoverage[day]?.ok,
											'coverage-bad': !dayCoverage[day]?.ok,
											'week-start': isWeekStart(day),
										}"
									>
										<b>{{ dayCoverage[day]?.ok ? "✓" : "!" }}</b
										><small v-if="!dayCoverage[day]?.ok"
											>У {{ dayCoverage[day]?.morning || 0 }} · В
											{{ dayCoverage[day]?.evening || 0 }}</small
										>
									</td>
									<td>{{ completeDays }}/{{ days.length }}</td>
								</tr>
							</tfoot>
						</table>
					</div>
				...
				<div class="legend">
					<span
						...
						:key="shift.name"
						:class="{
							'legend-morning': shift.shift_code === 'U',
							'legend-evening': shift.shift_code === 'V',
						}"
						><b>{{ shiftLabel(shift) }}</b> {{ shift.shift_name }} ·
						{{ shift.paid_hours }} ч</span
					><span class="legend-both"><b>У/В</b> обе смены у одного сотрудника</span
					><span
						><b>Прошедшие дни</b> защищены; изменение включается отдельной
						галочкой</span
					>
				</div>
			</div>
			...
			<div v-else-if="section === 'payroll'" class="team-panel">
				<div class="panel-title payroll-controls">
					<div>
						<h2>Расчёт зарплаты</h2>
						<p>Факт часов и личная выручка берутся из закрытых кассовых смен.</p>
					</div>
					<label>С <input v-model="payrollStart" type="date" /></label
					><label>По <input v-model="payrollEnd" type="date" /></label>
				</div>
				<div v-if="!payroll" class="team-empty">
					<b>Выберите точку и период, затем нажмите «Рассчитать»</b
					><span>Сохранение создаёт ведомость без проведения выплаты.</span>
				</div>
				<template v-else>
					<SmartDataTable
						:rows="payroll.rows"
						:entity-fields="entityFields"
						view-key="team.payroll"
						:selectable="false"
						empty-title="Нет данных за период"
					/>
					<div class="totals">
						<span
							>Начислено <b>{{ money(payroll.totals.gross) }}</b></span
						><span
							>НДФЛ <b>{{ money(payroll.totals.ndfl) }}</b></span
						><span
							>К выплате <b>{{ money(payroll.totals.net) }}</b></span
						><span
							>Полная стоимость <b>{{ money(payroll.totals.cost) }}</b></span
						>
					</div>
					<p v-if="payroll.name" class="saved">
						Ведомость сохранена: {{ payroll.name }}
					</p>
				</template>
			</div>
			...
			<div v-else-if="section === 'bonuses'" class="team-panel">
				<div class="panel-title">
					<div>
						<h2>Премии и игра</h2>
						<p>
							Отзывы, регистрации клуба, подарки и средний чек рассчитываются из
							кассовых данных.
						</p>
					</div>
				</div>
				<div v-if="!data.motivation_periods.length" class="team-empty">
					<b>Активной игры пока нет</b>
				</div>
				<div v-else class="game-list">
					<article v-for="game in data.motivation_periods" :key="game.name">
						<div>
							<h3>{{ game.title }}</h3>
							<p>
								{{ pointNames[game.business_point] }} · {{ game.start_date }} —
								{{ game.end_date }}
							</p>
						</div>
						<code>{{ game.rules_version }}</code
						><button
							class="button"
							:disabled="recalculating"
							@click="recalculate(game.name)"
						>
							Пересчитать
						</button>
					</article>
			...
			<div v-else class="team-panel">
				<div class="panel-title">
					<div>
						<h2>Кадры и документы</h2>
						<p>Трудоустройство, отпуска и кадровые документы сотрудников.</p>
					</div>
					<router-link class="button" to="/references/employees"
						>Карточки сотрудников</router-link
					>
				</div>
				<SmartDataTable
					:rows="hr.leaves"
					:entity-fields="entityFields"
					view-key="team.hr.leaves"
					:selectable="false"
					empty-title="Отпусков пока нет"
					empty-text="Оформите отпуск, компенсацию или больничный."
				/>
			</div>
		</template>
	</section>
...
.team-panel {
	...
	border-radius: 16px;
	background: #fff;
	box-shadow: 0 8px 26px rgba(38, 59, 25, 0.06);
}
.team-stats article {
	padding: 16px;
}
...
.schedule-grid thead .weekday-row th {
	top: 0;
	height: 24px;
	padding-block: 2px;
	color: #687462;
...
.schedule-grid thead .day-row th {
	top: 29px;
	height: 25px;
}
...
.schedule-grid select:disabled {
	cursor: not-allowed;
	color: #66705f;
	background: rgba(238, 241, 235, 0.72);
	border-color: #e1e5dc;
}
.schedule-grid .weekend {
	background: #e9edf2;
}
.schedule-grid .week-start {
	border-left: 2px solid #a9b4a3;
}
.schedule-grid td.shift-morning {
	background: #fff4a8 !important;
}
.schedule-grid td.shift-evening {
	background: #a9cef7 !important;
}
.schedule-grid td.shift-both {
	background: linear-gradient(135deg, #fff4a8 0 50%, #a9cef7 50% 100%) !important;
}
.schedule-grid .past-day {
...
.schedule-grid tfoot .employee-col {
	z-index: 3;
}
.schedule-grid tfoot small {
	display: block;
	white-space: nowrap;
	font-size: 9px;
}
...
.schedule-edit-controls label {
	display: flex;
	align-items: center;
	gap: 7px;
	font-weight: 700;
}
.schedule-edit-controls span {
	color: #7a8575;
}
.coverage-summary {
	display: flex;
