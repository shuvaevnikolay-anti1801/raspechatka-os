  ...
  <section class="page team-page">
    <ListPageHeader :title="title">
      <template #actions>
        ...
        <template v-if="section==='schedule'"><button class="button" :disabled="saving||!point||days.every(isPastDay)" @click="saveSchedule(false)">Сохранить</button><button class="button button-primary" :disabled="saving||!point||days.every(isPastDay)" @click="saveSchedule(true)">Опубликовать</button></template>
      ...
      <div v-else-if="section==='schedule'" class="team-panel schedule-panel">
        ...
        <template v-else>
          ...
          <div class="schedule-scroll">
            <table class="schedule-grid"><thead><tr><th class="employee-col">Сотрудник</th><th v-for="day in days" :key="day" :class="{ 'coverage-bad': !dayCoverage[day]?.ok, 'past-day': isPastDay(day) }">{{ day }}</th><th>Часы</th></tr></thead>
            <tbody><tr v-for="employee in selectedPointEmployees" :key="employee.name"><th class="employee-col">{{ employee.employee_name }}</th>
              <td v-for="day in days" :key="day" :class="{ 'coverage-bad-soft': !dayCoverage[day]?.ok, 'past-day': isPastDay(day) }"><select v-model="draft[cellKey(employee.name, day)]" :disabled="isPastDay(day)" :title="isPastDay(day) ? 'Прошедшие смены изменять нельзя' : isoDate(day)"><option value="">—</option><option v-for="shift in baseShifts" :key="shift.name" :value="shift.name">{{ shift.shift_code }}</option><option v-if="baseShifts.length === 2" :value="BOTH">У/В</option></select></td>
        ...
        <div class="legend"><span v-for="shift in baseShifts" :key="shift.name"><b>{{ shift.shift_code }}</b> {{ shift.shift_name }} · {{ shift.paid_hours }} ч</span><span><b>У/В</b> обе смены у одного сотрудника</span><span><b>Прошедшие дни</b> зафиксированы и недоступны для изменения</span></div>
      </div>
