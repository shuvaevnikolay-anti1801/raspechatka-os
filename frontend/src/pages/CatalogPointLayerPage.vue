...
async function saveAssortment(row) {
	...
	try {
		...
		feedback.value = filters.business_point === "__all__"
...
async function bulk(enabled) {
	...
	if ((!enabled || filters.business_point === "__all__") && !window.confirm(
	...
	try {
		...
		feedback.value = `${enabled ? "Включено" : "Выключено"} ${result.items} позиций; изменено ${result.updated} настроек в ${result.points} точках.`;
		await loadRows();
	} catch (exception) { error.value = exception.message; }
	finally { saving.value = ""; }
}
...
<template>
	<section class="page layer-page">
		...
		<div class="layer-workspace">
			<CatalogGroupSidebar :groups="options.groups" :selected="filters.catalog_group" :states="layer === 'assortment' ? groupStates : {}" @select="selectGroup" />
					...
					<tbody>
						<tr v-for="row in rows" :key="row.row_key || row.name">
							...
							<template v-if="layer === 'assortment'">
								<td v-if="filters.business_point !== '__all__'"><input v-model="row.enabled" type="checkbox" :disabled="!canEdit || saving === row.name" :true-value="1" :false-value="0" @change="saveAssortment(row)" /></td>
								<td v-else>
									<button class="assortment-state" :class="`assortment-state--${row.assortment_state}`" :disabled="!canEdit || saving === row.name" @click="setAggregated(row, row.assortment_state !== 'all')">
										{{ row.assortment_state === "all" ? "✓ Во всех" : row.assortment_state === "partial" ? `◐ В ${row.enabled_points} из ${row.point_count}` : "○ Нигде" }}
									</button>
								</td>
							</template>
...
.form-success { color:var(--green-dark); background:var(--green-soft); padding:9px 12px; border-radius:10px; }
.assortment-state { border:0; border-radius:9px; padding:7px 10px; cursor:pointer; background:#f3f4f1; color:var(--muted); }
.assortment-state--all { color:var(--green-dark); background:var(--green-soft); }
.assortment-state--partial { color:#8a5a00; background:#fff4d6; }
