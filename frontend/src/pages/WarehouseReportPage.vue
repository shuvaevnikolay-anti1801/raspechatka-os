...
const rows = ref([]), totals = ref({}), loading = ref(true), error = ref(""), proposalLoading = ref(false);
...
async function createPurchaseDrafts(){
  ...
  try{
    ...
    const message=[`Будет создано черновиков: ${proposal.groups_count}.`,`Товарных позиций: ${proposal.items_count}.`,`Количество: ${formatValue(proposal.total_quantity,"qty")}.`,proposal.unresolved?.length?`Без основного поставщика или доступа: ${proposal.unresolved.length}.`:"","Создать черновики заказов?"].filter(Boolean).join("\n");
...
<template><section class="page report-page"><ListPageHeader :title="config.title"><template #actions><button v-if="canCreatePurchase" class="button button-primary" :disabled="proposalLoading" @click="createPurchaseDrafts">{{ proposalLoading ? "Формируем…" : "Сформировать закупку" }}</button><button class="button button-secondary" @click="exportCsv">Экспорт CSV</button></template></ListPageHeader>
