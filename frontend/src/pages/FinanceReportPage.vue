...
const percent = (value) =>
	`${new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 1 }).format(Number(value || 0))}%`;
const points = computed(() =>
	options.points.filter(
		(item) =>
			!filters.value.business_entity ||
			item.business_entity === filters.value.business_entity
	)
);
const heading = computed(
	() =>
		({
			...
			settlements: "Задолженность поставщикам",
...
const filterFields = computed(() => [
	...(kind.value === "report" || kind.value === "overview"
		...
		: [
				{ key: "search", label: "Поиск", placeholder: "Товар или код", wide: true },
	...
	},
	{
		...
		type: "select",
		allLabel: "Все точки",
		options: points.value.map((item) => ({ value: item.name, label: item.point_name })),
	},
	...(kind.value === "profitability"
		? [
				{
					key: "catalog_group",
					label: "Группа",
...
async function load() {
	...
	error.value = "";
	try {
		let method = "get_financial_report",
			params = {
				month: `${filters.value.month}-01`,
		...
		if (kind.value === "settlements") {
			Object.assign(
				data,
				await call("raspechatka.api.supplier_settlements.get_supplier_debt", {
					business_entity: filters.value.business_entity,
					business_point: filters.value.business_point,
		...
		if (kind.value === "profitability") {
			method = "get_profitability";
			params = {
				from_date: filters.value.from_date,
				to_date: filters.value.to_date,
				business_entity: filters.value.business_entity,
				business_point: filters.value.business_point,
				catalog_group: filters.value.catalog_group,
				search: filters.value.search,
			};
		}
...
<template>
	<section class="page finance-page">
		...
		<div
			v-else-if="error && (kind === 'report' || kind === 'overview')"
			class="table-message error-message"
		>
			<strong>Не удалось сформировать отчёт</strong><span>{{ error }}</span
			><button @click="load">Повторить</button>
		</div>
		<template v-else-if="kind === 'report' || kind === 'overview'">
			<div class="finance-kpis">
				<article class="accent">
					<small>ВЫРУЧКА</small><b>{{ money(data.pnl?.revenue) }}</b
					><span
						>план {{ money(data.plan_fact?.revenue_plan) }} ·
						{{ percent(data.plan_fact?.revenue_attainment) }}</span
					>
				</article>
				<article>
					<small>РАСХОДЫ</small><b>{{ money(data.pnl?.expenses) }}</b
					><span>по проведённым платежам</span>
				</article>
				<article>
					<small>ЧИСТАЯ ПРИБЫЛЬ</small><b>{{ money(data.pnl?.net_profit) }}</b
					><span>рентабельность {{ percent(data.pnl?.margin) }}</span>
				</article>
				<article>
					<small>ДЕНЬГИ НА КОНЕЦ</small><b>{{ money(data.cash_flow?.closing_cash) }}</b
					><span>чистый поток {{ money(data.cash_flow?.net_cash_flow) }}</span>
				</article>
			</div>
			<div class="finance-report-grid">
				<article class="finance-panel">
					...
					<div class="statement-row">
						<span>Выручка</span><b>{{ money(data.pnl?.revenue) }}</b>
					</div>
					<div class="statement-row">
						<span>Расходы</span><b>−{{ money(data.pnl?.expenses) }}</b>
					</div>
					<div class="statement-row total">
						<span>Чистая прибыль</span><b>{{ money(data.pnl?.net_profit) }}</b>
					</div>
					<div
						v-for="row in data.articles"
						:key="row.article"
						class="statement-row detail"
					>
				...
				<article class="finance-panel">
					...
					<div class="statement-row">
						<span>Деньги на начало</span
						><b>{{ money(data.cash_flow?.opening_cash) }}</b>
					</div>
					<div
						v-for="(row, key) in data.cash_flow?.sections"
						:key="key"
						class="statement-row"
					>
						<span>{{
					...
					<div class="statement-row total">
						<span>Деньги на конец</span
						><b>{{ money(data.cash_flow?.closing_cash) }}</b>
					</div>
				</article>
				<article class="finance-panel">
					<header>
						<small>БАЛАНС</small>
						<h2>Финансовая позиция</h2>
					</header>
					<div class="statement-row">
						<span>Деньги</span><b>{{ money(data.balance?.cash) }}</b>
					</div>
					<div class="statement-row">
						<span>Запасы</span><b>{{ money(data.balance?.inventory) }}</b>
					</div>
					<div class="statement-row">
						<span>Оборудование</span><b>{{ money(data.balance?.equipment) }}</b>
					</div>
					<div class="statement-row">
						<span>Обязательства</span><b>−{{ money(data.balance?.liabilities) }}</b>
					</div>
					<div class="statement-row total">
						<span>Чистая позиция</span><b>{{ money(data.balance?.net_position) }}</b>
					</div>
				</article>
			</div>
		</template>
		<template v-else>
			<div v-if="kind === 'profitability' && !data.source_ready" class="integration-note">
				<b>Отчёт готов к данным кассы</b
				><span
					>Строки появятся после подключения продаж: выручка − возвраты − скидки −
					себестоимость.</span
				>
			</div>
			<SmartDataTable
				:rows="data.rows || []"
				:entity-fields="entityFields"
				:row-key="kind === 'settlements' ? 'supplier' : 'item'"
				:totals="kind === 'settlements' ? data.totals : undefined"
				:view-key="`finance.${kind}`"
				:loading="loading"
				:error="error"
				:selectable="false"
				empty-text="Измените период или добавьте связанные операции."
				@retry="load"
			/>
		</template>
	</section>
