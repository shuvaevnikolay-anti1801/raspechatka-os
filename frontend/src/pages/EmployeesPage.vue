...
const entityFields = defineEntityFields([
	...
	{ key: "employment_type", label: "Оформление" },
	{
		...
		format: (value) =>
			value
				? value.active
					? `${labelProfile(value.access_profile)} · активен`
					: "Отключён"
				: "Не выдан",
	},
	{
		...
		allLabel: "Все",
		options: [
			{ value: "1", label: "Работают" },
			{ value: "0", label: "Уволены / архив" },
		],
	},
]);
...
async function create() {
	...
	currentStep.value = 1;
	try {
		const r = await call("raspechatka.api.team.get_employee_editor");
		Object.assign(options, r);
		detail.value = { access: null };
	} catch (e) {
		error.value = e.message;
	}
}
async function open(row) {
	currentStep.value = 1;
	try {
		const r = await call("raspechatka.api.team.get_employee_editor", { name: row.name });
		Object.assign(options, r);
		detail.value = r;
...
function togglePoint(point) {
	const list = form.assigned_points || (form.assigned_points = []),
		i = list.findIndex((x) => x.business_point === point.name);
...
function toggleAccessPoint(name) {
	const i = accessForm.points.indexOf(name);
	if (i >= 0) accessForm.points.splice(i, 1);
	else accessForm.points.push(name);
}
async function save() {
	saving.value = true;
	formError.value = "";
...
async function continueOnboarding() {
	...
	if (currentStep.value === 1) {
		if (!form.last_name || !form.first_name || !form.phone) {
			formError.value = "Заполните фамилию, имя и телефон.";
			return;
		}
		currentStep.value = 2;
		return;
	...
	if (currentStep.value === 2) {
		if (!form.business_entity || !form.position || !form.employment_type) {
			formError.value = "Выберите работодателя, должность и тип оформления.";
			return;
		}
		if (!(form.assigned_points || []).length) {
			formError.value = "Назначьте хотя бы одну точку работы.";
			return;
		}
		if (await save()) currentStep.value = 3;
		return;
	}
	if (currentStep.value === 3) {
		if (await save()) currentStep.value = 4;
	}
}
...
async function refreshAccess() {
	const r = await call("raspechatka.api.team.get_employee_editor", { name: form.name });
	detail.value.access = r.access;
}
...
async function uploadPassport(event, field) {
	...
	if (!file) return;
	if (!form.name) {
		formError.value = "Сначала сохраните карточку сотрудника, затем прикрепите паспорт.";
		return;
	}
	if (file.type !== "application/pdf") {
		formError.value = "Для паспорта прикрепите файл PDF.";
		return;
	}
	...
	try {
		...
		body.append("docname", form.name);
		body.append("fieldname", field);
		...
		form[field] = result.message.file_url;
		await save();
	} catch (e) {
		formError.value = e.message;
	} finally {
...
<template>
	<section class="page employee-page">
		<ListPageHeader title="Сотрудники"
			><template #actions
		...
		<AppModal v-if="detail !== null" title="Карточка сотрудника" wide @close="detail = null">
			<form class="editor-form" @submit.prevent="save">
				<nav class="employee-steps" aria-label="Этапы заполнения карточки">
					<button
						v-for="step in steps"
						:key="step.number"
						type="button"
						:class="{
							...
							done: currentStep > step.number || (form.name && step.number < 3),
						}"
						@click="form.name || step.number <= 2 ? (currentStep = step.number) : null"
					>
				...
				<div class="step-intro">
					<b
						...
						{{ steps[currentStep - 1].title }}</b
					><span v-if="currentStep === 1">Введите минимум данных, чтобы начать.</span
					><span v-else-if="currentStep === 2"
						>Укажите условия, по которым сотрудник попадёт в график и расчёт
						зарплаты.</span
					><span v-else-if="currentStep === 3"
						>Дополните кадровые данные и сформируйте комплект документов.</span
					><span v-else>Доступ нужен не каждому сотруднику и выдаётся отдельно.</span>
				</div>
				<div v-show="currentStep === 1" class="form-section">
					...
					<div class="form-grid">
						...
						<label
							>Пол<select v-model="form.gender">
				...
				<div v-show="currentStep === 2" class="form-section">
					...
					<div class="form-grid">
						<label
							>Работодатель — ИП<select v-model="form.business_entity" required>
						...
						>
						<label
							>Должность<select v-model="form.position" required>
								<option value="">Выберите</option>
								<option
						...
						>
						<label
							>Тип оформления<select v-model="form.employment_type" required>
								<option>Трудовой договор</option>
								<option>ГПХ</option>
								<option>Самозанятый</option>
								<option>ИП</option>
								<option>Без оформления</option>
							</select></label
						>
					...
					<div class="point-picker">
						<label v-for="p in availablePoints" :key="p.name"
							><input
								type="checkbox"
								:checked="selectedPoints.includes(p.name)"
								@change="togglePoint(p)"
							/>
							{{ p.point_name }}</label
						>
					</div>
				...
				<div v-show="currentStep === 3" class="form-section">
					<h3>Данные для трудоустройства</h3>
					<div class="form-grid">
						<label
							>ИНН<input v-model="form.inn" inputmode="numeric" maxlength="12"
						/></label>
						<label
							>СНИЛС<input v-model="form.snils" inputmode="numeric" maxlength="14"
						/></label>
						<label
						...
						/></label>
						<label
							>Дата выдачи<input v-model="form.passport_issue_date" type="date"
						/></label>
						<label
				...
				<div v-show="currentStep === 3" class="form-section">
					...
					<p class="section-note">
						Прикрепляются два отдельных PDF. Для загрузки сначала сохраните новую
						карточку.
					</p>
					<div class="document-grid">
						<label
							...
							><a
								v-if="form.passport_main_file"
								:href="form.passport_main_file"
								target="_blank"
								>Открыть текущий файл</a
							><input
								type="file"
								accept="application/pdf"
								:disabled="!form.name || uploading"
								@change="uploadPassport($event, 'passport_main_file')"
							/><span v-if="uploading === 'passport_main_file'"
								>Загрузка…</span
							></label
						>
						<label
							...
							><a
								v-if="form.passport_registration_file"
								:href="form.passport_registration_file"
								target="_blank"
								>Открыть текущий файл</a
							><input
								type="file"
								accept="application/pdf"
								:disabled="!form.name || uploading"
								@change="uploadPassport($event, 'passport_registration_file')"
							/><span v-if="uploading === 'passport_registration_file'"
								>Загрузка…</span
							></label
						>
					</div>
				...
				<div v-show="currentStep === 3" class="form-section">
					<h3>Банковские реквизиты для зарплаты</h3>
					<div class="form-grid">
						<label class="span-2"
							>Получатель<input
								v-model="form.salary_recipient_name"
								:placeholder="form.employee_name"
						/></label>
						<label
							>БИК
							<div class="field-with-action">
								<input
									v-model="form.salary_bic"
									inputmode="numeric"
									maxlength="9"
								/><button
									class="button button-secondary"
									type="button"
									:disabled="lookingUpBank || !form.salary_bic"
									@click="lookupBank"
								>
									{{ lookingUpBank ? "Ищем…" : "Заполнить по БИК" }}
								</button>
							</div></label
						>
						<label class="span-2">Банк<input v-model="form.salary_bank_name" /></label>
						<label
							>Корреспондентский счёт<input
								v-model="form.salary_correspondent_account"
								inputmode="numeric"
								maxlength="20"
						/></label>
						<label class="span-2"
							>Счёт получателя<input
								v-model="form.salary_account"
								inputmode="numeric"
								maxlength="20"
						/></label>
					</div>
				</div>
				<div v-if="form.name && currentStep === 3" class="form-section">
					<div class="documents-heading">
						<div>
							<h3>Документы трудоустройства</h3>
							<p class="section-note">
								Комплект формируется из актуальных шаблонов и сохраняется приватно
								в карточке.
							</p>
						</div>
						<button
						...
						>
							{{
								generatingDocs ? "Формируем…" : "Сформировать документы для найма"
							}}
						</button>
					</div>
					<div v-if="form.documents?.length" class="generated-documents">
						<a
							...
							><span>{{ item.status }} · {{ item.issue_date }}</span
							><small>{{ item.notes }}</small></a
						>
					</div>
				...
				<div v-if="form.name && currentStep === 4" class="form-section access-section">
					<div class="access-title">
						<div>
							...
							<p v-else>
								{{ labelProfile(detail.access.access_profile) }} ·
								{{ detail.access.active ? "доступ активен" : "доступ отключён" }} ·
								{{ detail.access.invitation_status }}
							</p>
						</div>
					</div>
					<template v-if="!detail.access">
						...
						<div class="point-picker">
							<label v-for="p in employeePoints" :key="p.name"
								><input
									type="checkbox"
									:checked="accessForm.points.includes(p.name)"
									@change="toggleAccessPoint(p.name)"
								/>
								{{ p.point_name }}</label
							>
						</div>
					...
					<template v-else
						><div class="security-actions">
							<button
								v-if="!detail.access.active"
								type="button"
								class="button"
								@click="setAccess(1)"
							>
								Восстановить доступ</button
							><button v-else type="button" class="button" @click="setAccess(0)">
								Отключить доступ</button
							><button type="button" class="button button-primary" @click="grant">
								Создать новую ссылку входа
							</button>
						</div></template
					>
					<div v-if="invitation" class="invitation">
						<textarea :value="invitation" rows="6" readonly></textarea
						><button type="button" class="button" @click="copyInvitation">
...
.employee-steps button b {
	font-size: 13px;
}
.employee-steps button small {
	font-size: 11px;
	color: #7a8476;
}
.employee-steps button.active {
	border-color: #75a936;
	background: #f6faef;
}
.employee-steps button.active span,
.employee-steps button.done span {
	background: #75a936;
	color: #fff;
}
.step-intro {
