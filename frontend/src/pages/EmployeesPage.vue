...
async function create(){reset();currentStep.value=1;try{const r=await call("raspechatka.api.team.get_employee_editor");Object.assign(options,r);detail.value={access:null}}catch(e){error.value=e.message}}
async function open(row){currentStep.value=1;try{const r=await call("raspechatka.api.team.get_employee_editor",{name:row.name});Object.assign(options,r);detail.value=r;reset(JSON.parse(JSON.stringify(r.employee)));if(r.access){accessForm.access_profile=r.access.access_profile;accessForm.points=(r.access.assigned_points||[]).map(x=>x.business_point)}}catch(e){error.value=e.message}}
...
async function save(){
 ...
 try{const check=await call("raspechatka.api.hr_documents.validate_employee_requisites",{data:JSON.stringify(form)},{method:"POST"});if(!check.valid)throw new Error(check.errors.join(". "));const r=await call("raspechatka.api.team.save_employee",{data:JSON.stringify(form)},{method:"POST"});const step=currentStep.value;await load();await open({name:r.name});currentStep.value=step;return true}
 catch(e){formError.value=e.message;return false}finally{saving.value=false}
}
async function continueOnboarding(){
 formError.value="";
 if(currentStep.value===1){
  if(!form.last_name||!form.first_name||!form.phone){formError.value="Заполните фамилию, имя и телефон.";return}
  currentStep.value=2;return;
 }
 if(currentStep.value===2){
  if(!form.business_entity||!form.position||!form.employment_type){formError.value="Выберите работодателя, должность и тип оформления.";return}
  if(!(form.assigned_points||[]).length){formError.value="Назначьте хотя бы одну точку работы.";return}
  if(await save())currentStep.value=3;return;
 }
 if(currentStep.value===3){if(await save())currentStep.value=4}
}
...
<section class="page employee-page">
 ...
 <AppModal v-if="detail!==null" title="Карточка сотрудника" wide @close="detail=null">
  <form class="editor-form" @submit.prevent="save">
   ...
   <div class="step-intro"><b>Шаг {{currentStep}} из {{steps.length}} — {{steps[currentStep-1].title}}</b><span v-if="currentStep===1">Введите минимум данных, чтобы начать.</span><span v-else-if="currentStep===2">Укажите условия, по которым сотрудник попадёт в график и расчёт зарплаты.</span><span v-else-if="currentStep===3">Дополните кадровые данные и сформируйте комплект документов.</span><span v-else>Доступ нужен не каждому сотруднику и выдаётся отдельно.</span></div>
   <div v-show="currentStep===1" class="form-section"><h3>Сотрудник</h3><div class="form-grid">
   ...
   <div v-show="currentStep===2" class="form-section"><h3>Трудоустройство</h3><div class="form-grid">
   ...
   <div v-show="currentStep===3" class="form-section"><h3>Данные для трудоустройства</h3><div class="form-grid">
   ...
   <div v-show="currentStep===3" class="form-section"><h3>Паспорт — файлы</h3><p class="section-note">Прикрепляются два отдельных PDF. Для загрузки сначала сохраните новую карточку.</p><div class="document-grid">
   ...
   <div v-show="currentStep===3" class="form-section"><h3>Банковские реквизиты для зарплаты</h3><div class="form-grid">
  ...
  <template #footer><div class="footer-actions"><button class="button" @click="detail=null">Закрыть</button><button v-if="currentStep>1" class="button" type="button" @click="currentStep--">← Назад</button><button v-if="currentStep<4" class="button button-primary" type="button" :disabled="saving" @click="continueOnboarding">{{saving?"Сохраняем…":currentStep===2&&!form.name?"Создать карточку и продолжить":"Сохранить и продолжить →"}}</button><button v-else class="button button-primary" :disabled="saving" @click="save">{{saving?"Сохраняем…":"Сохранить сотрудника"}}</button></div></template>
 </AppModal>
</section>
</template>

<style scoped>
.employee-steps{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:14px}.employee-steps button{display:grid;grid-template-columns:32px 1fr;grid-template-rows:auto auto;column-gap:9px;text-align:left;padding:10px;border:1px solid #dfe5da;border-radius:10px;background:#fff;color:#53604f}.employee-steps button span{grid-row:1/3;display:grid;place-items:center;width:30px;height:30px;border-radius:50%;background:#edf1e9;font-weight:700}.employee-steps button b{font-size:13px}.employee-steps button small{font-size:11px;color:#7a8476}.employee-steps button.active{border-color:#75a936;background:#f6faef}.employee-steps button.active span,.employee-steps button.done span{background:#75a936;color:#fff}.step-intro{display:flex;gap:8px;flex-wrap:wrap;padding:12px 14px;border-radius:9px;background:#f4f6f1;color:#5f695b;margin-bottom:12px}.step-intro b{color:#273123}
.point-picker{display:flex;flex-wrap:wrap;gap:10px;margin:8px 0 16px}.point-picker label{display:flex;align-items:center;gap:7px;border:1px solid #dfe6d8;border-radius:9px;padding:9px 12px;background:#fff}.access-section{border:1px solid #dfe9d6;border-radius:12px;background:#f8fbf5;padding:16px}.access-title h3{margin:0}.access-title p{margin:5px 0 14px;color:#6f7d68}.security-actions{display:flex;gap:10px;flex-wrap:wrap}.invitation{display:grid;gap:10px;margin-top:14px}.invitation textarea{width:100%;resize:vertical}.form-section h4{margin:14px 0 6px}.footer-actions{display:flex;gap:10px;margin-left:auto}.section-note{color:#74806f;margin:-4px 0 12px}.document-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.document-grid label{display:grid;gap:8px;border:1px solid #e1e7dc;border-radius:10px;padding:14px}.document-grid a{color:#4f7d2d;font-size:13px}.documents-heading{display:flex;justify-content:space-between;gap:14px;align-items:flex-start}.generated-documents{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.generated-documents a{display:grid;gap:4px;padding:12px;border:1px solid #dfe7d9;border-radius:9px;color:inherit;text-decoration:none}.generated-documents span,.generated-documents small{color:#6d7768;font-size:12px}.document-grid input{padding:8px;border:1px dashed #cfd9c7;border-radius:8px}@media(max-width:700px){.employee-steps{grid-template-columns:1fr 1fr}.document-grid,.generated-documents{grid-template-columns:1fr}.documents-heading{flex-direction:column}}
</style>
