...
const entityFields = defineEntityFields([
  { key: "search", label: "Поиск", placeholder: "Контрагент или назначение", form: false, filter: false, table: false },
  ...
  { key: "direction", label: "Операция", type: "select", width: 120, options: [{ value: "Income", label: "Приход" }, { value: "Expense", label: "Расход" }], format: (value) => value === "Income" ? "Приход" : "Расход" },
  { key: "amount", label: "Сумма", type: "number", number: true, width: 140, format: money },
  { key: "processing_status", label: "Статус", type: "select", width: 150, options: [{ value: "New", label: "Новая" }, { value: "Review", label: "На проверке" }, { value: "Classified", label: "Классифицирована" }] },
]);
const operations = computed(() => data.operations.filter((row) =>
  ...
  (!filters.value.processing_status || row.processing_status === filters.value.processing_status)));
...
<template>
  <section class="page finance-page">
    ...
    <SmartFilterBar v-model="filters" :entity-fields="entityFields" view-key="finance.bank.operations" />
    <SmartDataTable :rows="operations" :entity-fields="entityFields" view-key="finance.bank.operations" :loading="loading" :error="error" empty-title="Операций пока нет" empty-text="После подключения и синхронизации здесь появится выписка." @open="review" @retry="load"><template #cell-processing_status="{row}"><span class="document-state" :class="`bank-${row.processing_status.toLowerCase()}`">{{row.processing_status}}</span></template></SmartDataTable>
