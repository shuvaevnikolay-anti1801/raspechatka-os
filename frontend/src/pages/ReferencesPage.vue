...
const filterFields = computed(() => [
	{ key: "search", label: "Поиск", placeholder: "Поиск по справочнику", wide: true },
	{
		...
		options: [
			...
			{ value: "0", label: reference.value === "entities" ? "Архивные" : "Неактивные" },
		],
	},
]);
...
function newReference() {
	...
	if (reference.value === "entities") {
		resetObject(form, {
			short_name: "",
			full_name: "",
			organization: options.organizations[0]?.name || "",
			active: 1,
			last_name: "",
			first_name: "",
			middle_name: "",
...
async function openReference(row) {
	formError.value = "";
	try {
		const result = await call("raspechatka.api.references.get_reference_detail", {
			reference: reference.value,
			name: row.name,
		});
		detail.value = result;
...
async function lookupInn() {
	...
	try {
		const result = await call("raspechatka.api.references.lookup_entity_by_inn", {
			inn: form.inn,
		});
		...
		>
			...
			>
				<div class="form-section">
					...
					<div class="form-grid">
						<label
							>Партнёр<select v-model="form.organization" required>
								<option
						...
						/></label>
						<label class="span-2"
							>Полное наименование<input v-model="form.full_name" required
						/></label>
					</div>
				</div>
				<div class="form-section">
						...
						<label class="span-2"
							...
							<div class="field-with-action">
								...
								/><button
									...
									type="button"
									:disabled="lookingUpInn || !form.inn"
									@click="lookupInn"
								>
									{{ lookingUpInn ? "Ищем…" : "Заполнить по ИНН" }}
								</button>
							</div></label
						>
						<p class="muted-note span-3">
				...
				<div class="form-section">
					...
					<div class="form-grid">
						<label
							>Налоговый режим<select v-model="form.tax_system">
								<option>ОСНО</option>
								<option>УСН Доход</option>
								<option>УСН Доход минус расход</option>
				...
				<div v-if="form.name" class="form-section">
					...
					<div
						v-if="detail.bank_accounts?.length"
						class="compact-list bank-account-list"
					>
						<div
							v-for="account in detail.bank_accounts"
							:key="account.name"
							:class="{ archived: !account.active }"
						>
							<b>{{ account.bank_name }}</b
							><span>{{ account.settlement_account }}</span
							><small
								>БИК {{ account.bic }} ·
								{{ account.active ? "Активен" : "В архиве" }}</small
							><span class="row-actions"
								><button
									class="text-button"
									type="button"
									@click="editBank(account)"
								>
									Изменить</button
								><button
									class="text-button"
									type="button"
									@click="setBankActive(account, account.active ? 0 : 1)"
								>
									{{ account.active ? "В архив" : "Восстановить" }}
								</button></span
							>
						</div>
					</div>
						...
						<label
							...
							<div class="field-with-action">
								...
								/><button
									class="button button-secondary"
									type="button"
									:disabled="lookingUpBic || !bankForm.bic"
									@click="lookupBic"
								>
									{{ lookingUpBic ? "Ищем…" : "Заполнить" }}
								</button>
							</div></label
						>
						<label class="span-2">Банк<input v-model="bankForm.bank_name" /></label>
						<label
							>Расчётный счёт<input
								v-model="bankForm.settlement_account"
								inputmode="numeric"
								maxlength="20" /></label
						><label
							>Корреспондентский счёт<input
								v-model="bankForm.correspondent_account"
								inputmode="numeric"
								maxlength="20" /></label
						><label>Валюта<input v-model="bankForm.currency" /></label>
						<label class="span-3"
							>Адрес банка<input v-model="bankForm.bank_address"
						/></label>
					</div>
					<div class="footer-actions">
						...
						>
							Отмена</button
						><button class="button button-secondary" type="button" @click="saveBank">
							{{ bankForm.name ? "Сохранить счёт" : "Добавить счёт" }}
						</button>
					</div>
				</div>
			...
			>
				...
				<div class="form-section">
					<h3>Контакты</h3>
					<div class="form-grid">
						<label>Город<input v-model="form.city" required /></label
						><label class="span-2"
							>Адрес<textarea v-model="form.address" rows="2" required></textarea>
						</label>
						<label>Телефон<input v-model="form.phone" /></label
						><label>E-mail<input v-model="form.email" type="email" /></label
				...
				<div class="form-section">
					...
					<div class="hours-list">
						<div v-for="day in form.working_hours" :key="day.weekday">
							<label class="check-field"
								><input
			...
			<div v-else class="warehouse-editor">
				<div class="warehouse-summary">
					<div>
						<span>Склад</span><b>{{ form.warehouse_name }}</b>
					</div>
					<div>
						<span>Точка продаж</span><b>{{ form.business_point }}</b>
					</div>
					<div>
						<span>Статус</span><b>{{ form.active ? "Активен" : "Выключен" }}</b>
					</div>
				</div>
				<section class="storage-section">
					<h3>Шкафы и места хранения</h3>
					<div v-if="detail.cabinets?.length" class="cabinet-grid">
						<article v-for="cabinet in detail.cabinets" :key="cabinet.name">
							<h4>{{ cabinet.cabinet_name }}</h4>
							<div
								v-for="place in cabinet.locations"
								:key="place.name"
								class="location-row"
							>
								{{ place.location_name }}
							...
							<button
								type="button"
								class="text-button"
								@click="locationForm.cabinet = cabinet.name"
							>
								＋ Добавить место
							</button>
						</article>
					...
					<div class="inline-form">
						<input
							v-model="cabinetForm.cabinet_number"
							type="number"
							min="1"
					...
					<div v-if="locationForm.cabinet" class="inline-form accent-inline">
						<input
							v-model="locationForm.location_name"
							placeholder="Название: Полка верхняя"
						/><button
							class="button button-secondary"
							type="button"
							@click="saveLocation"
						>
							Сохранить место</button
						><button
							class="text-button"
							type="button"
							@click="locationForm.cabinet = ''"
						>
							Отмена
						</button>
					</div>
				...
				<section class="storage-section">
					<h3>Адреса товаров</h3>
					<div v-if="detail.item_storage?.length" class="compact-list">
						<div v-for="item in detail.item_storage" :key="item.name">
							<b>{{ item.item }}</b
							><span>{{ item.full_address }}</span>
						</div>
					</div>
					<div class="inline-form storage-inline">
						<select v-model="storageForm.item">
						...
						><select v-model="storageForm.storage_location">
							<option value="">Выберите место</option>
							<option
								v-for="place in warehouseLocations"
								:key="place.name"
								:value="place.name"
							>
								{{ place.full_address }}
			...
			<template v-if="reference !== 'warehouses'" #footer>
				...
				<div class="footer-actions">
					<button class="button button-secondary" type="button" @click="detail = null">
						Отмена</button
					><button
						class="button button-primary"
						type="button"
