...
const expanded = ref(loadExpandedGroups());
...
const children = computed(() => {
	...
	for (const group of props.groups) {
		...
		result.get(parent).push(group);
	}
	for (const rows of result.values()) {
		rows.sort((a, b) => (a.group_name || "").localeCompare(b.group_name || "", "ru"));
	}
	return result;
});
...
const rows = computed(() => {
	...
	const visit = (parent, depth) => {
		for (const group of children.value.get(parent) || []) {
			if (visited.has(group.name)) continue;
			visited.add(group.name);
			const hasChildren = Boolean(children.value.get(group.name)?.length);
			result.push({ ...group, depth, hasChildren });
			if (hasChildren && expanded.value.has(group.name)) visit(group.name, depth + 1);
		}
	};
	visit("", 0);
	for (const group of props.groups) {
		if (!visited.has(group.name)) result.push({ ...group, depth: 0, hasChildren: false });
	}
	return result;
});
...
watch(
	...
	(selected) => {
		if (!selected) return;
		const byName = new Map(props.groups.map((group) => [group.name, group]));
		const next = new Set(expanded.value);
		let parent = byName.get(selected)?.parent_catalog_group;
		while (parent) {
			next.add(parent);
			parent = byName.get(parent)?.parent_catalog_group;
		}
		expanded.value = next;
		localStorage.setItem(storageKey, JSON.stringify([...next]));
	},
		...
		<div class="group-tree">
			...
			<div v-for="group in rows" :key="group.name" class="group-entry">
				<button
					...
					class="group-toggle"
					:style="{ left: `${8 + Math.min(group.depth, 5) * 18}px` }"
					type="button"
					:title="expanded.has(group.name) ? 'Свернуть' : 'Раскрыть'"
...
.group-add { width: 34px; height: 34px; border: 0; border-radius: 10px; background: var(--green-soft); color: var(--green-dark); font-size: 20px; cursor: pointer; }
.group-tree { padding: 8px; max-height: calc(100vh - 245px); overflow: auto; }
.group-entry { position: relative; }
.group-row { width: 100%; min-height: 40px; display: grid; grid-template-columns: 18px minmax(0, 1fr) auto; gap: 7px; align-items: center; padding: 7px 34px 7px 10px; border: 0; border-radius: 10px; background: transparent; color: var(--text); text-align: left; cursor: pointer; }
