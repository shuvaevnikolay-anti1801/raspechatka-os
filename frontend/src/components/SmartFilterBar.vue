...
const fields = computed(() => {
	...
	return configuredFields.value.map((field) => {
		...
		return {
			...
			...field,
			source: "configured",
			doctypeField: Boolean(schemaField),
		};
	});
});
...
const periodFieldPair = computed(() => {
	...
	for (const [fromKey, toKey] of knownPairs) {
		if (byKey.has(fromKey) && byKey.has(toKey))
			return { from: byKey.get(fromKey), to: byKey.get(toKey) };
	}
...
const operatorOptions = {
	text: [
		...
		{ value: "is_not_set", label: "не заполнено" },
	],
	exact: [
		{ value: "equals", label: "равно" },
		{ value: "not_equals", label: "не равно" },
...
function defaults() {
	return fields.value.filter((field) => field.default !== false).map((field) => field.key);
}
...
function setValue(key, value) {
	...
	if (
		periodFieldPair.value &&
		[periodFieldPair.value.from.key, periodFieldPair.value.to.key].includes(key)
	)
		next.__periodPreset = "";
...
function resolvePeriodPreset(filters) {
	return filters?.__periodPreset
		? { ...filters, ...periodValues(filters.__periodPreset) }
		: filters;
}
function setPeriod(preset) {
	emit("update:modelValue", { ...props.modelValue, ...periodValues(preset) });
}
function operatorKey(field) {
	return `${field.key}__operator`;
}
...
function dynamicCriteria() {
	return fields.value
		.filter((field) => field.doctypeField)
		.flatMap((field) => {
			const operator = selectedOperator(field),
				value = props.modelValue[field.key];
			if (needsValue(field) && (value === undefined || value === null || value === ""))
				return [];
	...
	await call(
		"raspechatka.api.references.save_view_preference",
		{
			...
			settings: JSON.stringify({
				...
				schema: fields.value.map((field) => field.key),
				bookmarks: bookmarks.value,
				lastFilters: props.modelValue,
				...extra,
			}),
		},
		{ method: "POST" }
...
async function reset() {
	...
	setDocumentFilterMatches(props.viewKey, null);
	await savePreference({ lastFilters: empty });
	emit("reset");
}
...
async function useBookmark(bookmark) {
	visible.value =
		bookmark.visible?.filter((key) => fields.value.some((field) => field.key === key)) ||
		defaults();
	const filters = resolvePeriodPreset(bookmark.filters);
	emit("update:modelValue", { ...props.modelValue, ...filters });
	await savePreference({ lastFilters: filters });
	await nextTick();
	await apply();
}
	...
	<section class="smart-filter">
		<div class="smart-filter-topline">
			...
			<div class="smart-filter-actions">
				...
				<button
					...
					aria-label="Настроить поля фильтра"
				...
				<button
					...
					aria-label="Сохранить фильтр как закладку"
			...
			<div v-if="settingsOpen" class="filter-field-popover">
				<label v-for="field in fields" :key="field.key"
					...
					/><span
						>{{ field.label
						}}<small v-if="field.child_table">табличная часть</small></span
					></label
				>
			</div>
		</div>
		<div class="smart-filter-grid">
			<label
				...
				:key="field.key"
				class="smart-filter-field"
				:class="{ wide: field.wide, dynamic: field.source === 'doctype' }"
			>
				<span>{{ field.label }}</span>
				<div class="filter-value">
					<select
						v-if="field.doctypeField"
						class="filter-operator"
						:value="selectedOperator(field)"
						@change="setValue(operatorKey(field), $event.target.value)"
					>
						<option
							v-for="operator in operators(field)"
							:key="operator.value"
					...
					<select
						v-if="field.type === 'select' && needsValue(field)"
						:value="modelValue[field.key]"
						@change="setValue(field.key, $event.target.value)"
					>
						<option :value="field.emptyValue ?? ''">
							{{ field.allLabel || "Все" }}
						</option>
						<option
				...
				<div
					v-if="periodFieldPair && field.key === periodFieldPair.from.key"
					class="filter-period-presets"
					aria-label="Быстрый выбор периода"
				>
					<button
						type="button"
						:class="{ active: modelValue.__periodPreset === 'today' }"
						@click.prevent.stop="setPeriod('today')"
					>
						Сегодня
					</button>
					<button
						type="button"
						:class="{ active: modelValue.__periodPreset === 'yesterday' }"
						@click.prevent.stop="setPeriod('yesterday')"
					>
						Вчера
					</button>
					<button
						type="button"
						:class="{ active: modelValue.__periodPreset === 'week' }"
						@click.prevent.stop="setPeriod('week')"
					>
						Неделя
					</button>
					<button
						type="button"
						:class="{ active: modelValue.__periodPreset === 'month' }"
						@click.prevent.stop="setPeriod('month')"
					>
						Месяц
					</button>
				</div>
		...
		<div v-if="bookmarks.length" class="filter-bookmarks">
			...
			><button
				...
				:key="bookmark.id"
				type="button"
				@click="useBookmark(bookmark)"
			>
				{{ bookmark.name
				}}<i title="Удалить" @click.stop="removeBookmark(bookmark.id)">×</i>
			</button>
		</div>
	</section>
</template>
