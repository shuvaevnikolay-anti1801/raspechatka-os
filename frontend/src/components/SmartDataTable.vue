  ...
  <div class="smart-table">
    ...
    <div class="table-shell">
      ...
      <table v-else>
        ...
        <tfoot v-if="Object.keys(totals).length"><tr><td v-if="selectable"></td><td v-for="column in visibleColumns" :key="column.key" :class="{ 'number-cell': column.number }"><strong v-if="column.primary">Итого</strong><strong v-else-if="totals[column.key]!==undefined">{{ column.format ? column.format(totals[column.key], totals) : totals[column.key] }}</strong></td></tr></tfoot>
      </table>
    </div>
