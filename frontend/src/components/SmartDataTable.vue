...
const props = defineProps({
  rows: { type: Array, default: () => [] }, columns: { type: Array, default: () => [] }, entityFields: { type: Array, default: null }, viewKey: { type: String, required: true },
...
const selected = ref([]), columnOrder = ref([]), selectedRows = ref([]), widths = ref({}), settingsOpen = ref(false), page = ref(1), pageSize = ref(25), ready = ref(false), sort = ref({ key: "", direction: "asc" });
const pageSizes = [25, 50, 100];
const preferenceKey = computed(() => `${props.viewKey}.table`);
const availableColumns = computed(() => props.entityFields ? deriveTableColumns(props.entityFields) : props.columns);
const visibleColumns = computed(() => columnOrder.value.map((key) => availableColumns.value.find((column) => column.key === key)).filter((column) => column && selected.value.includes(column.key)));
...
const effectiveTotals = computed(() => {
  ...
  return Object.fromEntries(availableColumns.value.filter((column) => column.number).map((column) => [column.key, filteredRows.value.reduce((total, row) => total + Number(row[column.key] || 0), 0)]));
});
let resizing = null, draggingKey = null;
...
async function savePreference() {
  ...
  await call("raspechatka.api.references.save_view_preference", { view_key: preferenceKey.value, settings: JSON.stringify({ columns: selected.value, columnOrder: columnOrder.value, schema: availableColumns.value.map((column) => column.key), widths: widths.value, pageSize: pageSize.value }) }, { method: "POST" });
}
async function loadPreference() {
  ready.value = false; selected.value = availableColumns.value.filter((column) => column.default !== false).map((column) => column.key); columnOrder.value = availableColumns.value.map((column) => column.key); widths.value = {}; pageSize.value = 25; page.value = 1;
  try {
    const preference = await call("raspechatka.api.references.get_view_preference", { view_key: preferenceKey.value });
    selected.value = reconcileVisible(preference.columns, availableColumns.value, "table", preference.schema);
    columnOrder.value = reconcileColumnOrder(preference.columnOrder || preference.columns, availableColumns.value);
    widths.value = Object.fromEntries(Object.entries(preference.widths || {}).filter(([key]) => availableColumns.value.some((column) => column.key === key)));
...
function beginDrag(event, column) { if (column.fixed) return; draggingKey = column.key; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", column.key); }
function dropColumn(event, target) {
  const source = draggingKey || event.dataTransfer.getData("text/plain"); draggingKey = null;
  ...
  columnOrder.value = movable; savePreference();
}
...
watch(() => props.viewKey, loadPreference); watch(availableColumns, () => { selected.value = reconcileVisible(selected.value, availableColumns.value, "table", columnOrder.value); columnOrder.value = reconcileColumnOrder(columnOrder.value, availableColumns.value); }); watch(() => props.currentPage, (value) => { page.value = value; }); watch(() => filteredRows.value.length, () => { if (page.value > pageCount.value) page.value = pageCount.value; });
onMounted(loadPreference); onBeforeUnmount(() => window.removeEventListener("pointermove", resize));
</script>

<template>
  <div class="smart-table">
    <div class="table-meta"><strong>{{ selectedRows.length ? `Выбрано: ${selectedRows.length}` : `${rowCount} записей` }}</strong><div class="column-settings"><button class="text-button" type="button" @click="settingsOpen=!settingsOpen">Настроить столбцы</button><div v-if="settingsOpen" class="column-popover"><label v-for="column in availableColumns" :key="column.key"><input type="checkbox" :checked="selected.includes(column.key)" @change="toggleColumn(column.key)" /><span>{{ column.label }}</span></label></div></div></div>
    <div class="table-shell">
      ...
      <table v-else>
        ...
        <thead><tr><th v-if="selectable" class="select-cell"><input type="checkbox" :checked="pageRows.length&&pageRows.every(row=>selectedRows.includes(row[rowKey]))" aria-label="Выбрать страницу" @change="togglePage" /></th><th v-for="column in visibleColumns" :key="column.key" @dragover.prevent @drop.prevent="dropColumn($event,column)"><button v-if="!column.fixed" class="column-drag-handle" type="button" draggable="true" title="Перетащить столбец" aria-label="Перетащить столбец" @click.stop @dragstart="beginDrag($event,column)">⋮⋮</button><span @click="changeSort(column.key)">{{ column.label }} <i v-if="sort.key===column.key" class="sort-mark">{{sort.direction==='asc'?'↑':'↓'}}</i></span><i class="column-resizer" @click.stop @pointerdown="beginResize($event,column)"></i></th></tr></thead>
