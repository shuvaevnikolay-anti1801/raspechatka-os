...
test("key and validation", () => {
	...
	assert.equal(
		api.validate("", "Strong-123!", "Strong-123!", false).message,
...
test("public errors never interpolate server details", () => {
	const raw = {
		message: "English secret",
		_server_messages: "traceback",
		exc_type: "InvalidResetKey",
	};
	...
	assert.match(
		api.publicError(400, { exc_type: "PasswordPolicyError", message: "secret" }, true),
		/требованиям/
	);
	assert.match(
		api.publicError(
			417,
			{ exc_type: "ValidationError", message: "<div>common password</div>" },
			true
		),
		/требованиям/
	);
