...
class DeletionContractTests(unittest.TestCase):
		...
		sys.modules.update(
			{
				...
				"raspechatka.access_contract": contract,
				"raspechatka.scope": scope,
			}
		)
	...
	def test_failed_handler_rolls_back_and_retry_reexecutes(self):
		...
		self.module.REGISTRY["sales_receipt"] = self.module.DeletionRule(
			old.doctype, old.area, old.scope, handler
		)
		try:
	...
	def test_purchase_order_blocks_receipts_and_allocations(self):
		...
		self.frappe.get_all.side_effect = lambda doctype, **kwargs: {
			"Stock Receipt": ["SR-1"],
			"Supplier Payment Allocation": ["PA-1"],
		}.get(doctype, [])
	...
	def test_employee_open_shift_blocks_archive_and_profile_is_preserved(self):
		...
		doc.get.side_effect = lambda key: {
			"system_user_profile": "PROFILE-1",
			"user": "web@example.test",
		}.get(key)
		self.frappe.get_all.side_effect = lambda doctype, **kwargs: (
	...
	def test_suppressed_pos_receipt_never_reaches_upsert(self):
		...
		fn = next(
			node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == "_ingest_receipt"
		)
		namespace = {
			"_required": lambda row, field: row[field],
			"is_external_event_suppressed": lambda source, external_id: (
				source == "POS" and external_id == "R1"
			),
			"frappe": self.frappe,
		}
	...
	def test_deleted_shift_cannot_be_recreated_by_pos_ingest_or_legacy_path(self):
		...
		fn = next(
			node
			for node in ast.parse(legacy.read_text()).body
		...
		with self.assertRaisesRegex(Exception, "Удалённая смена"):
			namespace["_get_or_create_legacy_shift"](
				types.SimpleNamespace(name="WORKPLACE"),
				"OLD-SHIFT",
	...
	def test_suppressed_moysklad_receipt_never_reaches_upsert(self):
		...
		tree = ast.parse(source.read_text())
		fn = next(
			node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == "_upsert_receipt"
		)
		namespace = {
