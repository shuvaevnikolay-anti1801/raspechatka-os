...
def test_route_and_invitation_contract():
	assert '{"from_route": "/update-password", "to_route": "os-update-password"}' in read("raspechatka/hooks.py")
...
def test_page_labels_and_no_raw_backend_messages():
	...
	for label in ("Создание пароля", "Текущий пароль", "Новый пароль", "Повторите пароль", "Создать пароль", "Продолжить"):
		assert label in html
	for forbidden in ("Set Password", "Invalid Link", "Passwords do not match", "_server_messages", "traceback", "responseJSON.message"):
