...
3. Первая загрузка данных запускается по событию `ready` от `SmartFilterBar` — только после восстановления schema, сохранённых полей и `lastFilters`. События `apply` и `reset` используются только для последующих действий пользователя.
...
Для server-side pagination первая загрузка должна дождаться двух независимых состояний: `SmartFilterBar.ready` и `SmartDataTable.ready(pageSize)`. Используйте общий readiness gate; нельзя одновременно запускать initial request из `onMounted`, filter `ready` и table `ready`.

Во время повторной загрузки уже полученные строки остаются в таблице. `SmartDataTable` показывает компактное состояние «Обновляем…»; полноэкранный/блокирующий loader предназначен только для первой загрузки без строк. Страницы с потенциально пересекающимися read-запросами должны игнорировать устаревший ответ через generation/request gate.
